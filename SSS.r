...
SSS <- function(
  ...
  if (SR_type == 7) {
    Input.draws <- as.data.frame(matrix(NA, nrow = reps, ncol = 11))
  }
  ...
  starter.new <- SS_readstarter(
    file.path(filepath, 'starter.ss'),
    verbose = FALSE
  )
  ...
  if (all(!is.na(ts_yrs))) {
    ...
    SS_writedat(
      dat.yrs,
      file.path(filepath, file.name[1]),
      overwrite = TRUE,
      verbose = FALSE
    )
  }
  ...
  while (i < reps + 1) {
    ...
    if (file.exists(paste(filepath, "/Report.sso", sep = ""))) {
    ...
    ctl.new <- SS_readctl(
      file.path(filepath, file.name[2]),
      use_datlist = TRUE,
      datlist = dat.new,
      verbose = FALSE
    )
    ...
    if (length(Dep.in) == 3) {
      ...
      if (Dep.in[1] == 2) {
        Dep.draw <- round(
          1 - rbeta.ab(1, 1 - Dep.in[2], Dep.in[3], 0.05, 0.95),
          2
        )
      }
      if (Dep.in[1] == 3) {
        Dep.draw <- round(rlnorm(1, log(Dep.in[2]), Dep.in[3]), 2)
      }
      if (Dep.in[1] == 4) {
        Dep.draw <- round(runif(1, Dep.in[2], Dep.in[3]), 2)
      }
      if (Dep.in[1] == 10) {
        Dep.draw <- round(rtnorm(1, Dep.in[2], Dep.in[3], 0.01, 1), 2)
      }
    ...
    if (FMSY_M.in[1] < 0 & BMSY_B0.in[1] < 0) {
      if (M.in[1] >= 0 & length(M.in) <= 6) {
        ...
        while (M.draw <= 0) {
          ...
          if (M.in[1] == 0) {
            M.draw <- round(rnorm(1, M.in[2], M.in[3]), 3)
          }
          if (M.in[1] == 3) {
            M.draw <- round(rlnorm(1, log(M.in[2]), M.in[3]), 3)
          }
          if (M.in[1] == 4) {
            M.draw <- round(runif(1, M.in[2], M.in[3]), 3)
          }
      ...
      if (sexes == T) {
        if (length(M.in) == 6) {
          ...
          while (M.draw.M < 0) {
            ...
            if (M.in[4] == 0) {
              M.draw.M <- round(rnorm(1, M.in[5], M.in[6]), 3)
            }
            if (M.in[4] == 3) {
              M.draw.M <- round(rlnorm(1, log(M.in[5]), M.in[6]), 3)
            }
            if (M.in[4] == 4) {
              M.draw.M <- round(runif(1, M.in[5], M.in[6]), 3)
            }
            Input.draws[i, 7] <- M.draw.M
          }
        }
        if (length(M.in) > 6) {
          Input.draws[i, 7] <- M.draw.M <- M.in[i, 2]
        }
      }
    }
    #Growth parameters
    ...
    #    {
    Covar_Linf_k <- Linf.k.cor *
      ((Linf.in[3] + 0.000001) * (k.in[3] + 0.000001)) #caluclate covariance matrix for multivariate sampling
    Linf_k_sigma <- matrix(
      c(
        (Linf.in[3] + 0.000001)^2,
        Covar_Linf_k,
        Covar_Linf_k,
        (k.in[3] + 0.000001)^2
      ),
      2,
      2
    ) #Set-up the variance-covariance matrix for multivariate sampling
    Linf_k_samps <- rtmvnorm(
      1,
      c(Linf.in[2], k.in[2]),
      Linf_k_sigma,
      lower = rep(0, length(c(Linf.in[2], k.in[2])))
    )
    ...
    if (sexes == T) {
      ...
      if (ctl.new$parameter_offset_approach == 1) {
        Covar_Linf_k <- Linf.k.cor *
          ((Linf.in[6] + 0.000001) * (k.in[6] + 0.000001)) #caluclate covariance matrix for multivariate sampling
        Linf_k_sigma <- matrix(
          c(
            (Linf.in[6] + 0.000001)^2,
            Covar_Linf_k,
            Covar_Linf_k,
            (k.in[6] + 0.000001)^2
          ),
        ...
        Linf_k_samps <- rtmvnorm(
          ...
          c(Linf.in[5], k.in[5]),
          Linf_k_sigma,
          lower = rep(0, length(c(Linf.in[5], k.in[5])))
        )
        ...
        L1.draw.M <- Input.draws[i, 8] <- VBGF(
          Linf.draw.M,
          k.draw.M,
          t0.draw.M,
          t0.draw
    ...
    #Steenpess
    if (
      h.in[1] >= 0 & length(h.in) == 3 & FMSY_M.in[1] < 0 & BMSY_B0.in[1] < 0
    ) {
      if (h.in[1] == 1) {
        h.draw <- round(rbeta.ab(1, h.in[2], h.in[3], 0.21, 0.99), 3)
      }
      if (h.in[1] == 2) {
        h.draw <- round(rbeta(1, h.in[2], h.in[3]), 3)
      }
      if (h.in[1] == 10) {
        h.draw <- round(rtnorm(1, h.in[2], h.in[3], 0.21, 0.99), 3)
      }
      if (h.in[1] == 30) {
        h.draw <- round(rlnormTrunc(1, log(h.in[2]), h.in[3], 0.21, 0.99), 2)
      }
      if (h.in[1] == 4) {
        h.draw <- round(runif(1, h.in[2], h.in[3]), 2)
      }
      Input.draws[i, 1] <- h.draw
    }
    if (length(h.in) > 3 & FMSY_M.in[1] < 0 & BMSY_B0.in[1] < 0) {
      Input.draws[i, 1] <- h.draw <- h.in[i]
    }
    #Get steepness for BH from FMSY
    if (h.in[1] == 99 | BH_FMSY_comp == T & FMSY_M.in[1] >= 0) {
      SRdat.new <- readLines(paste(
        filepath,
        "/h_BMSY_FMSY/BMSYB0.dat",
        sep = ""
      ))
      x <- 1
      while (x == 1) {
        ...
        if (FMSY_M.in[1] == 2) {
          FMSY_M.draw <- round(
            rbeta.ab(1, FMSY_M.in[2], FMSY_M.in[3], 0.0001, 10),
            3
          )
        }
        if (FMSY_M.in[1] == 10) {
          FMSY_M.draw <- round(
            rtnorm(1, FMSY_M.in[2], FMSY_M.in[3], 0.0001, 10),
            3
          )
        }
        if (FMSY_M.in[1] == 30) {
          FMSY_M.draw <- round(
            rlnormTrunc(1, log(FMSY_M.in[2]), FMSY_M.in[3], 0.0001, 10),
            3
          )
        }
        if (FMSY_M.in[1] == 4) {
          FMSY_M.draw <- round(runif(1, FMSY_M.in[2], FMSY_M.in[3]), 3)
        }
        if (length(FMSY_M.in) > 3) {
          FMSY_M.draw <- FMSY_M.in[i]
        }
        ...
        RUN.SS(
          paste(filepath, "/h_BMSY_FMSY/", sep = ""),
          ss.exe = "BMSYB0",
          ss.cmd = " -nox > out.txt"
        )
        if (
          file.exists(paste(filepath, "/h_BMSY_FMSY/bmsyb0.rep", sep = "")) ==
            TRUE
        ) {
          SRMQ_rep.new <- readLines(paste(
            filepath,
            "/h_BMSY_FMSY/bmsyb0.rep",
            sep = ""
          ))
          SRMQ_par[i] <- as.numeric(strsplit(
            readLines(paste(filepath, "/h_BMSY_FMSY/bmsyb0.par", sep = "")),
            split = " "
          )[[1]][11])
          steep_expo.out <- as.numeric(strsplit(SRMQ_rep.new[1], " ")[[1]])
          if (
            steep_expo.out[1] < 0.25 |
              steep_expo.out[1] >= 1 |
              steep_expo.out[2] > 10 |
              SRMQ_par[i] > 0.1
          ) {
            ...
            write(
              SRdat.new,
              paste(filepath, "/h_BMSY_FMSY/BMSYB0.dat", sep = "")
            )
            RUN.SS(
              paste(filepath, "/h_BMSY_FMSY/", sep = ""),
              ss.exe = "BMSYB0",
    ...
    if (FMSY_M.in[1] >= 0 & BMSY_B0.in[1] >= 0) {
      ...
      while (x == 1) {
        ...
        if (BMSY_B0.in[1] == 2) {
          BMSY_B0.draw <- round(
            rbeta.ab(1, BMSY_B0.in[2], BMSY_B0.in[3], 0.01, 0.99),
            2
          ) *
            100
        }
        if (BMSY_B0.in[1] == 10) {
          BMSY_B0.draw <- round(
            rtnorm(1, BMSY_B0.in[2], BMSY_B0.in[3], 0.01, 0.99),
            2
          ) *
            100
        }
        if (BMSY_B0.in[1] == 30) {
          BMSY_B0.draw <- round(
            rlnormTrunc(1, log(BMSY_B0.in[2]), BMSY_B0.in[3], 0.01, 0.99),
            2
          ) *
            100
        }
        if (BMSY_B0.in[1] == 4) {
          BMSY_B0.draw <- round(runif(1, BMSY_B0.in[2], BMSY_B0.in[3]), 2) * 100
        }
        if (length(BMSY_B0.in) > 3) {
          BMSY_B0.draw <- BMSY_B0.in[i]
        }
        Input.draws.MQs[i, 1] <- FMSY_M.draw
    ...
    if (Zfrac.Beta.in[1] >= 0) {
      if (Zfrac.Beta.in[1] == 10) {
        Zfrac.draw <- round(
          rtnorm(1, Zfrac.Beta.in[2], Zfrac.Beta.in[3], 0, 1),
          2
        )
      }
      if (Zfrac.Beta.in[1] == 30) {
        Zfrac.draw <- round(
          rlnormTrunc(1, log(Zfrac.Beta.in[2]), Zfrac.Beta.in[3], 0, 1),
          2
        )
      }
      if (Zfrac.Beta.in[1] == 4) {
        Zfrac.draw <- round(runif(1, Zfrac.Beta.in[2], Zfrac.Beta.in[3]), 2)
      }
      Input.draws[i, 1] <- Zfrac.draw
    }
    if (Zfrac.Beta.in[4] >= 0) {
      if (Zfrac.Beta.in[4] == 10) {
        Beta.draw <- round(
          rtnorm(1, Zfrac.Beta.in[5], Zfrac.Beta.in[6], 0, 10),
          2
        )
      }
      if (Zfrac.Beta.in[4] == 30) {
        Beta.draw <- round(
          rlnormTrunc(1, log(Zfrac.Beta.in[5]), Zfrac.Beta.in[6], 0, 10),
          2
        )
      }
      if (Zfrac.Beta.in[4] == 4) {
        Beta.draw <- round(runif(1, Zfrac.Beta.in[5], Zfrac.Beta.in[6]), 2)
      }
    ...
    if (doR0.loop[1] > 0) {
      ...
      if (
        is.na(as.numeric(strsplit(
          rep.new[grep(
            paste("Bratio_", as.numeric(dat.new$CPUE[2, 1]), sep = ""),
            rep.new
          )],
          " "
        )[[1]][2])) ==
      ...
      if (
        ...
        Dep.out.testR0 <- as.numeric(strsplit(
          rep.new[grep(
            paste("Bratio_", as.numeric(dat.new$CPUE[2, 1]), sep = ""),
            rep.new
          )],
      ...
      while (abs(Dep.out.testR0 - Dep.draw) > 0.01) {
        ...
        xx <- xx + 1
        if (xx == length(R0.explore)) {
          break
        }
      }
    }
    SSS.output.list[[i]] <- SS_output(
      paste(filepath, "/", sep = ""),
      covar = FALSE,
      verbose = FALSE,
      printstats = FALSE
    ...
    #      }
    if (i == 1) {
      SB.out <- TB.out <- SumAge.out <- SPR.out <- as.data.frame(matrix(
        NA,
        nrow = reps,
        ncol = length(SSS.output.list[[i]]$timeseries$SpawnBio)
      ))
      colnames(SB.out) <- colnames(TB.out) <- colnames(SumAge.out) <- colnames(
        SPR.out
      ) <- sb.years
      Dep.series.out <- as.data.frame(matrix(
        NA,
        nrow = reps,
        ncol = length(SSS.output.list[[i]]$annual_time_series$Depletion_std)
      ))
    ...
    OFL.out[[i]] <- SSS.output.list[[i]]$derived_quants[
      paste0("OFLCatch_", c((ts_yrs[2] + 1):(ts_yrs[2] + nforecasts))),
      "Value"
    ]
    ABC.out[[i]] <- SSS.output.list[[i]]$derived_quants[
      paste0("ForeCatch_", c((ts_yrs[2] + 1):(ts_yrs[2] + nforecasts))),
      "Value"
    ]
    ...
    if (SR_type == 3) {
      Quant.out[i, 9] <- SSS.output.list[[i]]$parameters[24, 3]
    }
    if (SR_type == 7) {
      Quant.out[i, 9] <- as.numeric(strsplit(
        rep.new[grep("SR_surv_Beta", rep.new) - 1],
        " "
      )[[1]][2])
    }
    if (SR_type == 7) {
      Quant.out[i, 29] <- as.numeric(strsplit(
        rep.new[grep("SR_surv_Beta", rep.new)],
        " "
      )[[1]][2])
    }
    if (SR_type == 8) {
      Quant.out[i, 9] <- as.numeric(strsplit(
        rep.new[grep("SR_RkrPower_steep", rep.new)],
        " "
      )[[1]][2])
    }
    if (SR_type == 8) {
      Quant.out[i, 29] <- as.numeric(strsplit(
        rep.new[grep("SR_RkrPower_gamma", rep.new)],
        " "
      )[[1]][2])
    }
    ...
    Quant.out[i, 12] <- as.numeric(SSS.output.list[[i]]$derived_quants[
      grep(
        paste("SSB_", ts_yrs[2], sep = ""),
        SSS.output.list[[i]]$derived_quants[, 1]
      ),
    ][2])
    Quant.out[i, 13] <- as.numeric(SSS.output.list[[i]]$derived_quants[
      grep(
        paste("Bratio_", ts_yrs[2], sep = ""),
        SSS.output.list[[i]]$derived_quants[, 1]
      ),
    ][2])
    Quant.out[i, 14] <- as.numeric(SSS.output.list[[i]]$derived_quants[
      grep("SSB_MSY", SSS.output.list[[i]]$derived_quants[, 1]),
    ][2])
    Quant.out[i, 15] <- as.numeric(SSS.output.list[[i]]$derived_quants[
      grep("B_MSY/SSB_unfished", SSS.output.list[[i]]$derived_quants[, 1]),
    ][2])
    if (
      !is.na(as.numeric(SSS.output.list[[i]]$derived_quants[
        grep("annF_MSY", SSS.output.list[[i]]$derived_quants[, 1]),
      ][2]))
    ) {
      Quant.out[i, 16] <- as.numeric(SSS.output.list[[i]]$derived_quants[
        grep("annF_MSY", SSS.output.list[[i]]$derived_quants[, 1]),
      ][2])
    }
    ...
    if (Dep.in[1] >= 0) {
      Quant.out[i, 21] <- as.numeric(strsplit(
        rep.new[grep(
          paste("Bratio_", as.numeric(dat.new$CPUE[2, 1]), sep = ""),
          rep.new
        )],
    ...
    Quant.out[i, 23] <- as.numeric(SSS.output.list[[i]]$derived_quants[
      grep(
        paste("F_", ts_yrs[2], sep = ""),
        SSS.output.list[[i]]$derived_quants[, 1]
      ),
    ][2]) /
      as.numeric(SSS.output.list[[i]]$derived_quants[
        grep("annF_MSY", SSS.output.list[[i]]$derived_quants[, 1]),
      ][2])
    Quant.out[i, 24] <- SSS.output.list[[i]]$likelihoods_used[11, 1]
    warnings.out <- readLines(paste(filepath, "/warning.sso", sep = ""))
    num.warning <- as.numeric(strsplit(
      warnings.out[grep("N warnings", warnings.out)[1]],
      " "
    )[[1]][4])
    ...
    if (Dep.in[1] >= 0) {
      if (
        FMSY_M.in[1] < 0 &
          BMSY_B0.in[1] < 0 &
          abs(Quant.out[i, 20] - Quant.out[i, 21]) < 0.01 &
          Quant.out[i, 24] < 0.0001
      ) {
        i <- i + 1
      }
      if (
        ...
        if (h.in[1] == 99) {
          if (
            abs(Quant.out[i, 20] - Quant.out[i, 21]) < 0.01 &
              Quant.out[i, 24] < 0.0001 &
              (abs(Quant.out[i, 14] - Input.draws.MQs[i, 3]) /
                Input.draws.MQs[i, 3]) <
                0.6
          ) {
            i <- i + 1
            ...
            if (n.bad == 0) {
              Quant.out.bad <- Quant.out[i, ]
            }
            if (n.bad > 0) {
              Quant.out.bad <- rbind(Quant.out.bad, Quant.out[i, ])
            }
            n.bad <- n.bad + 1
            if (
              length(M.in) > 6 |
          ...
          if (
            abs(Quant.out[i, 20] - Quant.out[i, 21]) < 0.01 &
              Quant.out[i, 24] < 0.0001 &
              (abs(Quant.out[i, 14] - (Input.draws.MQs[i, 2]) / 100) /
                (Input.draws.MQs[i, 2] / 100)) <
                0.5
          ) {
            i <- i + 1
          } else {
    ...
    if (Dep.in[1] < 0) {
      if (Quant.out[i, 24] < 0.0001) {
        ...
        par_line[1] <- 1
        starter.new[grep("ss.par", starter.new)] <- paste(
          par_line,
          collapse = " "
        )
        write(starter.new, paste(filepath, "/starter.ss", sep = ""))
        ...
        TB.out[i, ] <- SSS.output.list[[i]]$timeseries$Bio_all
        SumAge.out[i, ] <- SSS.output.list[[i]]$timeseries$Bio_smry
        Dep.series.out[i, ] <- SSS.output.list[[
          i
        ]]$annual_time_series$Depletion_std
    ...
    ii <- ii + 1
    if (file.exists(paste(filepath, "/Report.sso", sep = ""))) {
      file.remove(paste(filepath, "/Forecast-report.sso", sep = ""))
    }
    if (file.exists(paste(filepath, "/Report.sso", sep = ""))) {
      file.remove(paste(filepath, "/Report.sso", sep = ""))
    }
    colnames(Quant.out) <- colnames(Quant.out.bad) <- c(
      "M_f",
      "L1_f",
      "Linf_f",
      "k_f",
      ...
      "Dep.Exp",
      "R0_init",
      paste("F_", ts_yrs[2], "/FMSY", sep = ""),
      "Crash_penalty",
      "Rick_gamma"
    )
    if (ncol(Input.draws) == 10) {
      colnames(Input.draws) <- c(
        "h",
        "Dep",
        "M_f",
        "L1_f",
    ...
    if (SR_type >= 8 | h.in[1] == 99 | BH_FMSY_comp == T) {
      ...
      colnames(Input.draws)[c(ltcolnames - 1, ltcolnames)] <- c(
        "Beta",
        "Obj_fxn"
      )
    }
    ...
    colnames(OFL.out.save) <- colnames(ABC.out.save) <- c(
      (ts_yrs[2] + 1):(ts_yrs[2] + nforecasts)
    )
    Spp.quant.out <- list(
      Input.draws,
      Quant.out,
      ...
      Quant.out.bad,
      ii - 1,
      (as.numeric(end.time) - as.numeric(start.time)) / 60
    )
    names(Spp.quant.out) <- c(
      "Priors",
      "Posteriors",
      "SB_series",
      "Rel_Stock_status_series",
      "Total_Biomass",
      "Summary_Biomass",
      "OFL",
      "ABC",
    ...
    save(SSS.output.list, file = paste(filepath, "/SSSoutput.rds", sep = ""))
    save(SSS.out, file = paste(filepath, "/SSS_out.rds", sep = ""))
  }
