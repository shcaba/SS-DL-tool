...
Sensi_plot_horiz <- function(model.summaries,
  ...
  dev.quants.labs <- data.frame(
    c(
      "SB0", paste0("SSB_", current.year),
      paste0("Bratio_", current.year), "MSY_SPR", "F_SPR"
    ),
    dev.quants, "Derived quantities"
  )
  AICs <- 2 * model.summaries[["npars"]] + (2 * as.numeric(model.summaries[["likelihoods"]][
    1,
    1:model.summaries[["n"]]
  ]))
  ...
  Like.parm.quants <- rbind(
    AIC.out, survey.like, survey.lambda,
...
profile_multi <- function(
    dir, oldctlfile = "control.ss_new", masterctlfile = lifecycle::deprecated(),
    ...
    prior_check = TRUE, exe = "ss3", verbose = TRUE,
    ...) {
  orig_wd <- getwd()
  on.exit(setwd(orig_wd))
  if (lifecycle::is_present(masterctlfile)) {
    lifecycle::deprecate_warn(
      when = "1.46.0", what = "profile(masterctlfile)",
  ...
  if (npars == 1) {
    ...
    if (any(unlist(lapply(profilevec, FUN = length)) !=
      n)) {
      stop(
        "Each element in the 'profilevec' list should have length ",
        n
      )
    }
    if (verbose) {
  ...
  for (i in whichruns) {
    ...
    if (!overwrite & file.exists(newrepfile)) {
      ...
      onegood <- FALSE
      if (file.exists("Report.sso") & file.info("Report.sso")$size >
        0) {
        onegood <- TRUE
  ...
  if (onegood) {
    ...
    bigtable <- cbind(
      profilevec[whichruns, ], converged[whichruns],
      liketable
    )
