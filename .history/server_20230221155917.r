...
##########################
  ...
  pathModelDiag <- reactive({
      shinyDirChoose(input, "Model_diags_dir", roots=roots, filetypes=c('', 'txt'))
      return(parseDirPath(roots, input$Model_diags_dir))
    })
#Used to have as.numeric(input$tabs)==4
 observeEvent(as.numeric(input$tabs)==7,{      
  output$model_diag_pick<-renderUI({
      pickerInput(
      inputId = "myModelDiag",
      label = "Choose model to evaluate",
      choices = list.files(pathModelDiag()),
      options = list(
        `actions-box` = TRUE,
        size = 12,
        `selected-text-format` = "count > 3"
