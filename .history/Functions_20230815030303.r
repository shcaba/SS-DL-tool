...
mod_sums.temp1<-mod_sums.temp2<-model.summaries$likelihoods_by_fleet[1,]
mod_sums.temp1[1:length(mod_sums.temp1),]<-mod_sums.temp2[1:length(mod_sums.temp2),]<-NA
for(t in 1:model.summaries$n)
  {
    mod.labs.in<-subset(model.summaries$likelihoods_by_fleet,model==t)$Label
    add.labs<-mod.like.names[!mod.like.names%in%mod.labs.in]
    mod_sums.temp1%>% slice(rep(1:n(), each = length(add.labs)))    
    mod_sums.temp1[,1]<-t
    mod_sums.temp1[,2]<-add.labs
    model.summaries$likelihoods_by_fleet<-rbind(model.summaries$likelihoods_by_fleet,mod_sums.temp1)
  }
