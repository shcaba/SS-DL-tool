...
#Run SSS
        if(all(is.na(input$Depl_mean_sss)|input$Depl_mean_sss<0)&is.na(input$status_year))
      {
      #Throw warning if not enough selectivity inputs
         sendSweetAlert(
          session = session,
          title = "Stock Status and/or year input missing",
          text = "Please check to see if you have provided an input for relatives stock status >0 and year of that relative stock status.",
          type = "error")
         remove_modal_spinner()
         shiny::stopApp()
      }

if(input$Depl_mean_sss<0)
