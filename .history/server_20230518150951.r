...
{
  cuts1 <- data.frame(Ref="", vals=c(-1))
  cuts2 <- data.frame(Ref="L50", vals=c(L50()))
  cuts3 <- data.frame(Ref="Linf", vals=c(Linf()))
...
#        facet_wrap(sex~year, scales="free_y",ncol=5) + 
        ...
        geom_vline(xintercept = c(-1,L50(),Linf()),
                    ...
                    show.legend = TRUE)+
        xlim(0,NA)+
        annotate("text", x=L50(), y=-0.5, label= "L50")+
        annotate("text", x=Linf(), y=-0.5, label= "Linf")
