...
output$fit.mod.summary <- renderText({
 				fit.mod.summary<-summary(fit_model)
 				return(fit.mod.summary)	
			})
...
output$pairs_slow <- renderPlot({
      ...
      pairs_admb(fit_model, pars=1:10, order='slow')
      dev.off()
      }     
})
...
output$pairs_fast <- renderPlot({
      ...
      pairs_admb(fit_model, pars=1:10, order='fast')
      dev.off()
      }     
})
