...
output$AdvancedSS_addcomms<- renderUI({ 
        fluidRow(column(width=6, prettyCheckbox(
        inputId = "add_comms", 
        label = HTML('<p>Select metadata <a href=https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/12-2010.1>SS run commands</a> (e.g., -hess_step 5)</p>'),
        shape = "round", outline = TRUE, status = "info"))) 
  }) 
...
output$AdvancedSS_addcomms_user<- renderUI({ 
        fluidRow(column(width=6, prettyCheckbox(
        inputId = "add_comms", 
        label = paste0("Add additional",tags$a(href="javascript:window.open('SS_commands.html', '_blank','width=600,height=400')", " SS run commands")," (e.g., -hess_step 5)"),
        shape = "round", outline = TRUE, status = "info"))) 
  }) 
