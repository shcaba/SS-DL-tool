      ...
      #  }
       Lt.dat.plot<-rv.Lt$data %>%  
		    ...
		    mutate(Year = factor(year),
               ...
               #Lnu=-1)
       browser()
       Lt.dat.plot.L50<-Lt.dat.plot.Linf<-data.frame(year=1,expand.grid(unique(Lt.dat.plot$fleet),unique(Lt.dat.plot$sex)),name=00,value=00,Year=1,vline=-100)
       colnames(Lt.dat.plot.L50)<-colnames(Lt.dat.plot.Linf)<-c("year","fleet","sex","name","value","Year","vline")
