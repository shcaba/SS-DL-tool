...
######################
    ...
    if (!is.null(Age.comp.data))
    {
      # if(ncol(Age.comp.data)-8!=Plus_age)
      # {
      #    sendSweetAlert(
      #     session = session,
      #     title = "Age data input warning",
      #     text = "Your in the plus group input does not match the max age in the age composition data. Please makes sure these match.",
      #     type = "error")
      #    remove_modal_spinner()
      #    stopApp()
      # }
       ...
       if(ncol(data.frame((rv.AgeErr$data)))-8!=(Plus_age+1))
      {
         sendSweetAlert(
          ...
          text = "The ageing error  match the max age in the age composition data. Please makes sure these match.",
          type = "error")
         remove_modal_spinner()
         stopApp()
      }
