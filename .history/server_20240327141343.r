...
#        annotate("text",x=if_else(is.na(L50()),-1,L50())*1.1, y=5, label= "L50%",color="darkgreen")+
    ...
    {  
        ...
        Lt.Gr.plot<-rv.Lt$data %>%  
		    rename_all(tolower) %>%  
		    dplyr::select(-nsamps) %>%  
		    pivot_longer(c(-year, -fleet, -sex)) %>%  
		    mutate(Year = factor(year),
		           name = as.numeric(gsub("[^0-9.-]", "", name)))
       Lt.Gr.plot.Linf<-data.frame(year=1,expand.grid(unique(Lt.Gr.plot$fleet),unique(Lt.Gr.plot$sex)),name=00,value=00,Year=1,hline=-100,Linf.lab="Linf")
       Lt.Gr.plot.L50<-data.frame(year=1,expand.grid(unique(Lt.Gr.plot$fleet),unique(Lt.Gr.plot$sex)),name=00,value=00,Year=1,hline=-100,Linf.lab="L50")
       colnames(Lt.Gr.plot.L50)<-colnames(Lt.Gr.plot.Linf)<-c("year","fleet","sex","name","value","Year","vline","Label")
       if(!is.na(Linf())){Lt.Gr.plot.Linf$vline[Lt.Gr.plot.Linf$sex==0|Lt.Gr.plot.Linf$sex==1]<-Linf()}
       if(!is.na(Linf())){Lt.Gr.plot.Linf$vline[Lt.Gr.plot.Linf$sex==2]<-Linf_m_in()}
       if(!is.na(L50()))Lt.Gr.plot.L50$vline[Lt.Gr.plot.L50$sex==0|Lt.Gr.plot.L50$sex==1]<-L50()
       ...
       output$LtSelplot<-renderPlot({ 
        ggplot(lt.bins.sels())+ 
          ...
          geom_smooth(method=loess,se=FALSE,aes(Year,Sel,col=SelType))+
		    geom_hline(data=Lt.Gr.plot.L50,
                    aes(yintercept = hline,linetype="L50"),
                    colour = "darkgreen",
                    na.rm = TRUE)+
                    scale_linetype_manual(name = NULL, values = 3)+
        new_scale("linetype") +
        geom_hline(data=Lt.Gr.plot.Linf,
                    aes(yintercept = hline,linetype="Linf"),
                    colour = "#d26678",
                    na.rm = TRUE)+
                    scale_linetype_manual(name = NULL, values = 1)+
