...
#        annotate("text",x=if_else(is.na(L50()),-1,L50())*1.1, y=5, label= "L50%",color="darkgreen")+
    ...
    {  
       ...
       lt.bins.sels.in$L50<--100
       lt.bins.sels.in$Linf<--100
       if(!is.na(Linf())){lt.bins.sels.in$Linf[lt.bins.sels.in$Sex==0|lt.bins.sels.in$Sex==1]<-Linf()}
       if(!is.na(Linf())){lt.bins.sels.in$Linf[lt.bins.sels.in$Sex==2]<-Linf_m_in()}
       if(!is.na(L50()))lt.bins.sels.in$L50[lt.bins.sels.in$Sex==0|lt.bins.sels.in$Sex==1]<-L50()
       output$LtSelplot<-renderPlot({ 
        if (is.null(rv.Lt$data)) return(NULL)

          LtSelplot.out<-ggplot(lt.bins.sels())+ 
