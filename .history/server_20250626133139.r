...
output$Male_parms_inputs_M_SSS<- renderUI({
  if(input$male_parms_SSS){
       dropdownButton(
          ...
          numericInput("M_m_mean_sss", "Mean or lower bound for uniform", value=NA,min=0, max=10000, step=0.001),
          numericInput("M_m_SD_sss", "SD or upper bound for uniform", value=0.44,min=0, max=10000, step=0.001),
          circle = FALSE, right=TRUE, status = "danger", icon = icon("skull-crossbones"), width = "300px",label="Natural mortality"
            )
    }
}) 
...
########
    ...
    show_modal_spinner(spin="flower",color=wes_palettes$Zissou1[2],text="Model run in progress")
		if(input$Data_wt=="None or Current weighting"){DataWT_opt<-"none"}
...
#RUN SS3 MODELS
    ...
    if(is.null(input$no_hess)){
      cmd.in<-""
      if(!is.null(input$add_comms)){if(input$add_comms==TRUE){cmd.in=input$add_comms_in}}
...
#        }
      ...
      show_modal_spinner(spin="flower",color=wes_palettes$Rushmore[2],text="Preparing ensemble plots")
       ...
       gg5<-ggplot(SPR_t,aes(Model.labs,Metric,fill=Model))+
        ...
        gg1
        ggsave(paste0(Ensemble_model_dir_out,"/Ensemble_comp_plots.png_SOinit"),width=20,height=10)
        gg2
        ggsave(paste0(Ensemble_model_dir_out,"/Ensemble_comp_plots.png_SOterm"),width=20,height=10)
        gg3
        ggsave(paste0(Ensemble_model_dir_out,"/Ensemble_comp_plots.png_RSS"),width=20,height=10)
        gg4
        ggsave(paste0(Ensemble_model_dir_out,"/Ensemble_comp_plots.png_F"),width=20,height=10)
        gg5
        ggsave(paste0(Ensemble_model_dir_out,"/Ensemble_comp_plots.png_SO_1_SPR"),width=20,height=10)
      ...
      max.yr<-max(Ensemble_SO_plot$Year)
      min.yr<-min(Ensemble_SO_plot$Year)
      ...
      SO.ts.plot<-ggplot(Ensemble_SO_plot,aes(Year,SO,fill=Year))+
        ...
        scale_x_continuous(n.breaks = floor((max.yr-min.yr)/10))+
