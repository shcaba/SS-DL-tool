...
########################
  ...
  pathModEff <- reactive({
      shinyDirChoose(input, "ModEff_dir", roots=roots, filetypes=c('', 'txt'))
...
#   })
#print(pathModEff())
observeEvent(req(input$run_ADnuts&!is.null(input$myModEff)&as.numeric(input$tabs)==12),{
...
#setwd()
setwd(pathModEff())
file.copy(getwd(),recursive=TRUE,overwrite=TRUE)
setwd('..')
file.rename(paste0("Scenarios/sssexample_BH"), paste0("Scenarios/",input$Scenario_name))
