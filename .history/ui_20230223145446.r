...
########################
 ...
 shinyjs::hidden(wellPanel(id="Modeff_panel",
    ...
    tags$ul(tags$li(h5(p(em("No u-turn (Nuts). This method can quickly run Bayesian models once an efficient model is established."))))),
    ...
    br(),
    h5(em("Once the model specification is finalized, you can")),
    tags$ul(tags$li(h5(p(em("Re-run with Hessian to get asymptotic variance estimates "))))),
    tags$ul(tags$li(h5(p(em("Re-optimize, then consider using the NUTS option with 1000 iterations or keep running the RWM option until convergence criteria are reached by increasing the thinning value and/or number of iterations"))))),
    br(),
    h5(em("Good model convergence is considered when ESS>200 and Rhat <1.1")),
    h5(em("The complete model will be a Bayesian model")),
    br(),
   awesomeRadio(
