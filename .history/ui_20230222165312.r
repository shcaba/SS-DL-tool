...
########################
  ...
  shinyjs::hidden(wellPanel(id="Modeff_panel",
    ...
    h5(em("There are two main Bayesian methods to choose from:")),
    h5(p("Random walk Metropolis (RWM). This method is useful to quickly explore parameter behavior.")),
    h5(p("No u-turn (Nuts). This method can quickly run Bayesian models one an efficient model is established.")),
    h5(em("It is recommended to first run the RWM using 2000 iterations and thin = 10, then use the pairs plots to look for parameters that don't change value across kept draws")),
    h5(em("Parameters that don't move should be fixed in the model, and the model optimized again")),
    h5(em("Run the RWM model again and continue looking for non-moving parameters.")),
    h5(em("Once the model is finalized, you can consider using the NUTS model with 1000 iterations or keep running the RWM model until it meets convergence criteria by increasing the thinning value and or number of iterations.")),
    h5(em("Good convergence considered when ESS>200 and Rhat <1.1")),
    
    radioGroupButtons(
   inputId = "ModEff_choice",
   label = "Method", 
    choices = c("RWM", "Nuts", "Choice 3"),
   status = "primary"
),
