...
output$Selplot <- renderPlot({ 
    ...
    if(input$Sel_choice=="Logistic")
    {
      ...
      browser()
       ...
       Sel.out<-data.frame(Bin=Sel.out[,1],Sel=Sel.out[,2],Fleet=paste0("Fleet: ", fleetnames.ct[1]))
       if(length(Sel50)>1)
       {
        ...
        Sel.out.temp<-data.frame(Bin=Sel.out.temp[,1],Sel=Sel.out.temp[,2],Fleet=paste0("Fleet: ", fleetnames.ct[ii]))
