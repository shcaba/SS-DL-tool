...
observeEvent(req(!is.null(rv.Lt$data)), {
      shinyjs::show(output$lt_comp_sel_plots_label<-renderText({"Suggested selectivity starting values based on length compositions by year. Sel95 is based on the model of the composition; Sel50 is based on the midpoint of the mode and first size of recorded lengths."}))
  })
    ...
    {  
      output$LtSelplot<-renderPlot({ 
        ...
        ggplot(lt.bins.sels())+ 
          ...
          geom_smooth(method=lm,se=FALSE,formula=x~y,aes(Year,Sel,col=SelType))+
