 ...
 fluidRow(column(width=10,checkboxInput("user_model","Use existing model files?",FALSE))),
  ...
  h5(em("To do sensitivity runs for pre-existing models, make a copy of the folder and re-name it, then make desired changes in the data and/or control files.")),
    ...
    h4(strong("Life history inputs")),
        ...
        h5(em("Male")),
         fluidRow(column(width=6,checkboxInput("male_parms","Males specific values?",FALSE,style = "font-weight: 500;")),
                  column(width=6,checkboxInput("male_offset","Males offset from females (log(m/f))?",FALSE,style = "font-weight: 500;"))),
         h5(em("If estimating any female life history parameters and you want males to equal the females estimated values, use the offset option and pre-specify the male parameter(s) to 0 to ensure males = females. If you don't do this, the males values will stay at the female starting values.")),        
