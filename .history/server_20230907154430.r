...
browser()
Est.sel.tranformed<-Model.output$parameters[grep("Size_",rownames(Model.output$parameters)),]
if(length(grep("peak_",rownames(Model.output$parameters)))>0)
  {
    Est.sel.peak<-Model.output$parameters$Value[grep("peak_",rownames(Model.output$parameters))]
    Est.sel.tranformed$Value[grep("peak_",rownames(Model.output$parameters))]<-Est.sel.peak
    colnames(Est.sel.tranformed)[grep("peak_",rownames(Model.output$parameters))]<-"Length at Peak Selectivity"
  }

if(length(grep("ascend_",rownames(Model.output$parameters)))>0)
  {
    Est.sel.Sel50<-Model.output$parameters$Value[grep("ascend_",rownames(Model.output$parameters))]
    Est.sel.tranformed$Value[grep("ascend_",rownames(Model.output$parameters))]<-Est.sel.peak-sqrt(-exp(Est.sel.Sel50)*log(0.5))
    colnames(Est.sel.tranformed)[grep("ascend_",rownames(Model.output$parameters))]<-"Length at 50% Selectivity"
  }

if(length(grep("top_logit",rownames(Model.output$parameters)))>0)
  {
    Est.sel.Decline1<-Model.output$parameters$Value[grep("top_logit",rownames(Model.output$parameters))]
    Est.sel.tranformed$Value[grep("top_logit",rownames(Model.output$parameters))]<-((max(data.file$lbin_vector)-Est.sel.peak-bin.width)/-exp(Est.sel.Decline1))+(Est.sel.peak+bin.width)
    colnames(Est.sel.tranformed)[grep("top_logit",rownames(Model.output$parameters))]<-"Length at 1st declining selectivity"
  }

if(length(grep("descend",rownames(Model.output$parameters)))>0)
  {
    Est.sel.Width<-Model.output$parameters$Value[grep("descend",rownames(Model.output$parameters))]
    Est.sel.tranformed$Value[grep("descend",rownames(Model.output$parameters))]<-exp(Est.sel.Width)
    colnames(Est.sel.tranformed)[grep("descend",rownames(Model.output$parameters))]<-"Width of declining selectivity"
  }


if(length(grep("end_logit",rownames(Model.output$parameters)))>0)
  {
    Est.sel.Final<-Model.output$parameters$Value[grep("end_logit",rownames(Model.output$parameters))]
    Est.sel.tranformed$Value[grep("end_logit",rownames(Model.output$parameters))]<-(1/(1-exp(Est.sel.Final)))-0.000001
    colnames(Est.sel.tranformed)[grep("end_logit",rownames(Model.output$parameters))]<-"Selectivity at max bin size"
  }
