...
Est.sel.tranformed<-Model.output$estimated_non_dev_parameters[grep("peak_",rownames(xx$estimated_non_dev_parameters)),]
if(length(grep("peak_",rownames(xx$estimated_non_dev_parameters)))>0)
  {
    Est.sel.peak<-Model.output$estimated_non_dev_parameters$Value[grep("peak_",rownames(xx$estimated_non_dev_parameters))]
    Est.sel.tranformed$Value[grep("peak_",rownames(xx$estimated_non_dev_parameters))]<-Est.sel.peak
    colnames(Est.sel.tranformed)[grep("peak_",rownames(xx$estimated_non_dev_parameters))]<-"Length at Peak Selectivity"
  }

if(length(grep("ascend_",rownames(xx$estimated_non_dev_parameters)))>0)
  {
    Est.sel.Sel50<-Model.output$estimated_non_dev_parameters$Value[grep("ascend_",rownames(xx$estimated_non_dev_parameters))]
    Est.sel.tranformed$Value[grep("ascend_",rownames(xx$estimated_non_dev_parameters))]<-Est.sel.peak-sqrt(-exp(Est.sel.Sel50)*log(0.5))
    colnames(Est.sel.tranformed)[grep("ascend_",rownames(xx$estimated_non_dev_parameters))]<-"Length at 50% Selectivity"
  }

if(length(grep("top_logit",rownames(xx$estimated_non_dev_parameters)))>0)
  {
    Est.sel.Decline1<-Model.output$estimated_non_dev_parameters$Value[grep("top_logit",rownames(xx$estimated_non_dev_parameters))]
    Est.sel.tranformed$Value[grep("top_logit",rownames(xx$estimated_non_dev_parameters))]<-((max(data.file$lbin_vector)-Est.sel.peak-bin.width)/-exp(Est.sel.Decline1))+(Est.sel.peak+bin.width)
    colnames(Est.sel.tranformed)[grep("top_logit",rownames(xx$estimated_non_dev_parameters))]<-"Length at 1st declining selectivity"
  }

if(length(grep("descend",rownames(xx$estimated_non_dev_parameters)))>0)
  {
    Est.sel.Width<-Model.output$estimated_non_dev_parameters$Value[grep("descend",rownames(xx$estimated_non_dev_parameters))]
    Est.sel.tranformed$Value[grep("descend",rownames(xx$estimated_non_dev_parameters))]<-exp(Est.sel.Width)
    colnames(Est.sel.tranformed)[grep("descend",rownames(xx$estimated_non_dev_parameters))]<-"Width of declining selectivity"
  }


if(length(grep("end_logit",rownames(xx$estimated_non_dev_parameters)))>0)
  {
    Est.sel.Final<-Model.output$estimated_non_dev_parameters$Value[grep("end_logit",rownames(xx$estimated_non_dev_parameters))]
    Est.sel.tranformed$Value[grep("end_logit",rownames(xx$estimated_non_dev_parameters))]<-(1/(1-exp(Est.sel.Final)))-0.000001
    colnames(Est.sel.tranformed)[grep("end_logit",rownames(xx$estimated_non_dev_parameters))]<-"Selectivity at max bin size"
  }
