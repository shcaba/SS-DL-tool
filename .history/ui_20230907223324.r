...
shinyjs::hidden(wellPanel(id="panel_SS_LH_fixed_est_tog",
       ...
       h5(p("Natural mortality is an often difficult value to obtain. Consider using",tags$a(href="https://connect.fisheries.noaa.gov/natural-mortality-tool/", "The Natural Mortality Tool"), " (ctl+left click on the link to open the tool) to either obtain natural mortality values or developing a prior for use in estimating natural mortality. The Github repository for it can be found",tags$a(href="https://github.com/shcaba/Natural-Mortality-Tool", "here")," (ctl+left click on the link to open the repo).")),
       br(),
...
#      tags$ul(tags$li(h5(p(em("If using only length or age data, starting model year is based on earliest year minus age at 95% Linf"))))),
    ...
    #fluidRow(column(width=10,checkboxInput("advance_ss_click","Advanced SS options",FALSE))),
      ...
      fluidRow(style = "padding-right:50px;padding-left:50px;padding-top:-1000px; padding-bottom:0px;",h6("Software to calculate ageing error matrices from multiple age reads is available ",tags$a(href="https://github.com/pfmc-assessments/nwfscAgeingError", "here"),"(ctl+left click to open the repo).")),
