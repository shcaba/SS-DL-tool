...
#        }
      ...
      show_modal_spinner(spin="flower",color=wes_palettes$Rushmore[2],text="Preparing ensemble plots")
       ...
       gg3<-ggplot(Bratio_t,aes(Model.labs,Metric,fill=Model))+
        ...
        scale_y_continuous(limits = c(0, NA))+
        geom_hline(yintercept=c(Ensm_TRP,Ensm_LRP))+
        theme(legend.position = "none")
      ...
      browser()
      xlab.yr.short<-NA
      xlab.yr.short[Ensemble_SO_plot$Year%in%as.character(seq(min.yr,max.yr,floor((max.yr-min.yr)/10)))]<-as.character(seq(min.yr,max.yr,floor((max.yr-min.yr)/10)))
