...
#      tags$ul(tags$li(h5(p(em("If using only length or age data, starting model year is based on earliest year minus age at 95% Linf"))))),
    ...
    h5(p(strong("Advanced SS commands can be found "))),
    ...
    #fluidRow(column(width=10,checkboxInput("advance_ss_click","Advanced SS options",FALSE))),
      ...
      popify(uiOutput("AdvancedSS_addcomms"),"Custom SS run commands","Click the advanced SS commands link above to get options. One interesting option is -hess_step which attempts to make the model gradient 0. This should be run once a final model is found. Additional SS command can be found",tags$a(href="javascript:window.open('SS_commands.html', '_blank','width=600,height=400')", " here")),
      popify(uiOutput("AdvancedSS_addcomms_comms"),"Custom SS run commands","Click the advanced SS commands link above to get options. One interesting option is -hess_step which attempts to make the model gradient 0. This should be run once a final model is found."),
