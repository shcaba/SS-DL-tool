...
#Run MCMC model with either rwm or nuts
 if(input$ModEff_choice=="RWM")
   {
     fit_model<- sample_rwm(model=m, path=p, iter=input$iter, warmup=0.25*iter,
                       chains=chains, thin=input$thin, duration=duration.in)
   }

  if (input$ModEff_choice=="Nuts") 
  {
    fit_model <- sample_nuts(model=m, path=p,  iter=input$iter, warmup=0.25*iter, 
          chains=4, cores=4,control=list(metric='mle', max_treedepth=5))
  }

summary(fit_model)

output$pairs_slow <- renderPlot({
      {
...
save(fit.model,file=paste(p,"/fit.model.RData"))

    remove_modal_spinner()

if(input$run_stanout){launch_shinyadmb(fit_model)}
