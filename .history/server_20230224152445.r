...
output$AdvancedSS_addcomms<- renderUI({ 
        fluidRow(column(width=6, prettyCheckbox(
        inputId = "add_comms", label = paste0("Add additional",tags$a(href="javascript:window.open('SS_commands.html', '_blank','width=600,height=400')", " SS run commands")," (e.g., -hess_step 5)"),
        shape = "round", outline = TRUE, status = "info"))) 
  }) 
  ...
  output$AdvancedSS_addcomms_comms <- renderUI({ 
    if(!is.null(input$add_comms)){       
      ...
      fluidRow(column(width=12, textInput("add_comms_in", "Enter additional run commands", value="")))        
                 ...
                 as.character(actionLink(inputId = 'action_link', label = 'SS run commands')))
output$AdvancedSS_addcomms_user<- renderUI({ 
        fluidRow(column(width=6, prettyCheckbox(
        inputId = "add_comms", label = HTML("Add additional", tags$a(href="javascript:window.open('SS_commands.html', '_blank','width=600,height=400')", " SS run commands")," (e.g., -hess_step 5)"),
        shape = "round", outline = TRUE, status = "info"))) 
  }) 
...
#   })
 ...
 observeEvent(req(input$run_ADnuts&as.numeric(input$tabs)==12),{
 
 browser()
 modeff.mod.dir<-ModEff_dir #pathModEff()
 modeff.dir<-dirname(modeff.dir)
 modeff.name<-paste0(basename(modeff.dir),"_",input$ModEff_choice)
 dir.create(file.path(modeff.dir,modeff.name))
 file.copy(modeff.mod.dir,file.path(modeff.dir,modeff.name),recursive=TRUE,overwrite=TRUE)

#optimize model
show_modal_spinner(spin="flower",color=wes_palettes$Rushmore[1],text=paste0("Run initial optimization"))
system(paste0(file.path(modeff.dir,modeff.name),"/ss -nox -mcmc 100 -hbf"))
remove_modal_spinner()
...
#   {
#     fit_model<- sample_rwm(model=m, path=p, iter=iter, warmup=0.25*iter,
#                       chains=chains, thin=thin, duration=duration.in)=
#   }

#   if (input$ModEff_choice=="Nuts") 
#   {
#     fit_model <- sample_nuts(model=m, path=p,  iter=iter, warmup=0.25*iter, 
