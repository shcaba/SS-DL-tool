...
shinyServer(function(input, output, session) {
  ...
  saveInputs <- function(input, bookmarkPath, bookmarkURL, session) {
    session$doBookmark()
    bookmarkInputPath <- file.path(
      dirname(bookmarkPath),
      bookmarkURL,
      "input.rds"
    )
  ...
  observe({
    req(input$file33)
    req(!rv.AgeErr$clear)
    rv.AgeErr$data <- fread(
      input$file33$datapath,
      check.names = FALSE,
      header = FALSE,
      data.table = FALSE
    )
  ...
  observeEvent(
    ...
    )),
    {
      ...
      if (!is.null(rv.Ct$data)) {
        ct.flt <- c(1:(ncol(rv.Ct$data) - 1))
      }
      if (!is.null(rv.Lt$data)) {
        lt.flt <- rv.Lt$data[, 3]
      }
      if (!is.null(rv.Age$data)) {
        age.flt <- rv.Age$data[, 3]
      }
      if (!is.null(rv.Index$data)) {
  ...
  observeEvent(
    req(
      ((as.numeric(input$tabs) * 2) / 2) < 4 &
        all(!is.null(c(rv.Lt$data, rv.Age$data)), is.null(rv.Ct$data)) &
        any(is.null(input$user_model), !input$user_model)
    ),
    {
      shinyjs::show("Bookmark_panel")
  ...
  observeEvent(
    req(
      ((as.numeric(input$tabs) * 3) / 3) < 4 &
        all(
          any(input$est_parms == FALSE, input$est_parms2 == FALSE),
          any(
            all(!is.null(rv.Lt$data), !is.null(rv.Ct$data)),
            all(!is.null(rv.Age$data), !is.null(rv.Ct$data)),
    ...
    ),
    {
      ...
      shinyjs::hide("panel_Ct_F_LO")
      if (any(!is.null(rv.Lt$data), !is.null(rv.Age$data))) {
        shinyjs::show("panel_data_wt_lt")
      } else {
        (shinyjs::hide("panel_data_wt_lt"))
      }
  ...
  observeEvent(
    req(
      ((as.numeric(input$tabs) * 4) / 4) < 4 &
        all(
          input$est_parms == TRUE,
          any(
            all(!is.null(rv.Lt$data), !is.null(rv.Ct$data)),
            all(!is.null(rv.Age$data), !is.null(rv.Ct$data)),
            all(!is.null(rv.Index$data), !is.null(rv.Ct$data))
  ...
  output$Wt_fleet_Ct <- renderUI({
    ...
    fluidRow(column(
      ...
      textInput(
        "Wt_fleet_Ct",
        "Relative catch values",
        value = paste(
          rep(
            1,
            length(unique(na.omit(c(ct.flt, lt.flt, age.flt, index.flt)))[
              unique(na.omit(c(ct.flt, lt.flt, age.flt, index.flt))) > 0
            ])
          ),
          collapse = ","
  ...
  output$Eq_Ct_fleet <- renderUI({
    fluidRow(column(
      ...
      textInput(
        "Eq_Ct_fleet",
        "Equilibrium catch by fleet",
        value = paste(rep(0, ncol(rv.Ct$data) - 1), collapse = ",")
      )
    ))
  })
  ...
  output$Male_parms_inputs3 <- renderUI({
    if (input$male_parms) {
      fluidRow(column(
        width = 6,
        textInput("CV_lt_m", "CV at length (young then old)", value = "0.1,0.1")
      ))
    }
  })
    ...
    if (input$male_parms_SSS) {
      dropdownButton(
        selectInput(
          "M_m_prior_sss",
          "Prior type",
          c("lognormal", "normal", "uniform", "no prior")
        ),
        numericInput(
          "M_m_mean_sss",
          "Mean or lower bound for uniform",
          value = NA,
          min = 0,
          max = 10000,
          step = 0.001
  ...
  output$Rec_options6 <- renderUI({
    if (input$rec_choice) {
      fluidRow(column(
        ...
        selectInput(
          "RecDevChoice",
          "Recruit deviation option",
          c(
            "1: Devs sum to zero",
            "2: Simple deviations",
            "3: deviation vector",
            "4: option 3 plus penalties"
          ),
          selected = "3: deviation vector"
        )
      ))
    }
  })
      ...
      fluidRow(
        column(
          ...
          selectInput(
            ...
            selected = "3: Catch fxn of SSB, buffer on catch"
          )
        ),
  ...
  output$Forecasts <- renderUI({
    if (input$Forecast_choice) {
      fluidRow(
        ...
        column(
          width = 6,
          textInput("forecast_buffer", "Control rule buffer", value = "1")
        )
      )
    }
  })
  ...
  output$AdvancedSS_nohess <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "no_hess",
        label = "Turn off variance estimation (speeds up runs)",
        shape = "round",
        outline = TRUE,
        status = "info",
        value = TRUE
  ...
  output$AdvancedSS_nohess_user <- renderUI({
    ...
    fluidRow(column(
      width = 6,
      prettyCheckbox(
        inputId = "no_hess_user",
        label = "Turn off Hessian (speeds up runs, but no variance estimation)",
        shape = "round",
        outline = TRUE,
        status = "info",
        value = TRUE
  ...
  output$AdvancedSS_addcomms_comms <- renderUI({
    if (!is.null(input$add_comms)) {
      if (input$add_comms) {
        fluidRow(column(
          width = 12,
          textInput("add_comms_in", "Enter additional run commands", value = "")
        ))
      }
    }
  })
  ...
  output$AdvancedSS_noplots <- renderUI({
    ...
    fluidRow(column(
      width = 6,
      prettyCheckbox(
        inputId = "no_plots_tables",
        label = "Turn off plots",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_plots_RP_inputs <- renderUI({
    ...
    textInput(
      "plot_RPs_inputs",
      "Plot target and limit reference points",
      value = "0.4,0.25"
    )
    # }
    #}
  ...
  output$AdvancedSS_par <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "use_par",
        label = "Use par file (i.e., parameter file from previous run)?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_phase0 <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "use_phase0",
        label = "Turn off estimation of all parameters (phase = 0)?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_datanew <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "use_datanew",
        label = "Use the data_echo.ss_new file?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_controlnew <- renderUI({
    ...
    fluidRow(column(
      width = 6,
      prettyCheckbox(
        inputId = "use_controlnew",
        label = "Use the control.ss_new file?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_forecastnew <- renderUI({
    ...
    fluidRow(column(
      width = 6,
      prettyCheckbox(
        inputId = "use_forecastnew",
        label = "Use the forecast.ss_new file?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_GT1 <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "GT1",
        label = "Use only one growth type (default is 5)",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_GT5_SSS <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "GT5",
        label = "Use 5 growth types (default is 1)",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_Sex3options <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "Sex3options",
        label = "Maintain sex ratio in biological compositions",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_Indexvar <- renderUI({
    ...
    fluidRow(column(
      ...
      prettyCheckbox(
        inputId = "Indexvar",
        label = "Estimate additional variance on each abundance index?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_Ctunits <- renderUI({
    fluidRow(column(
      width = 12,
      prettyCheckbox(
        inputId = "Ct_units_choice",
        label = "Specify non-biomass catch units for each fleet.",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
    ...
    if (!is.null(input$Ct_units_choice)) {
      if (input$Ct_units_choice) {
        fluidRow(column(
          ...
          textInput(
            "fleet_ct_units",
            "Enter catch units for each fleet",
            value = ""
          )
        ))
      }
  ...
  output$AdvancedSS_Ctunits_SSS <- renderUI({
    fluidRow(column(
      width = 12,
      prettyCheckbox(
        inputId = "Ct_units_choice_SSS",
        label = "Specify catch units for each fleet?",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
    ...
    if (!is.null(input$Ct_units_choice_SSS)) {
      if (input$Ct_units_choice_SSS) {
        fluidRow(column(
          ...
          textInput(
            "fleet_ct_units_SSS",
            "Enter catch units for each fleet (1=biomass; 2=numbers)",
            value = ""
          )
        ))
      }
  ...
  output$AdvancedSS_retro_choice <- renderUI({
    fluidRow(column(
      width = 6,
      prettyCheckbox(
        inputId = "Retro_choice",
        label = "Do retrospective runs? Input minus from current year",
        shape = "round",
        outline = TRUE,
        status = "info"
      )
  ...
  output$AdvancedSS_Ltbin <- renderUI({
    # if(input$advance_ss_click){
    if (!is.null(rv.Lt$data)) {
      bin.step <- as.numeric(colnames(rv.Lt$data)[7]) -
        as.numeric(colnames(rv.Lt$data)[6])
    }
    if (is.null(rv.Lt$data)) {
      bin.step <- 2
    }
    ...
    fluidRow(
      ...
      column(
        ...
        numericInput(
          "lt_min_bin",
          "minimum bin",
          value = as.numeric(colnames(rv.Lt$data)[6]),
          min = 0,
          max = 10000,
          step = 0.01
        )
      ...
      column(
        ...
        numericInput(
          "lt_max_bin",
          "maximum bin",
          value = 2 * (round((Linf() + (Linf() * 0.2326)) / 2)) + 2,
          min = 0,
          max = 10000,
          step = 0.01
        )
    ...
    if (input$scenario_description) {
      fluidRow(column(
        ...
        textAreaInput(
          ...
          strong(
            "Write a description of your scenario in the text box below. This will go in the comment section at the top of your control and data files."
          ),
          value = ""
        )
      ))
  ...
  FleetNs <- reactive({
    ...
    fleetnum <- rep(
      1,
      max(rv.Lt$data[, 3], rv.Age$data[, 3], rv.Index$data[, 3])
    )
  ...
  Nages <- reactive({
    ...
    if (
      all(c(
        is.null(input$M_f),
        is.null(input$M_f_fix),
        is.null(input$M_f_mean),
        is.null(input$M_f_mean_sss),
        is.null(rv.Age$data)
      ))
    ) {
      return(NULL)
    ...
    if (!is.na(input$M_f_fix)) {
      Nages <- ceiling(5.4 / input$M_f_fix)
    }
    if (!is.na(input$M_f_mean)) {
      Nages <- ceiling(5.4 / input$M_f_mean)
    }
    if (!is.na(input$M_f_mean_sss)) {
      Nages <- ceiling(5.4 / input$M_f_mean_sss)
    }
    if (!is.null(rv.Age$data)) {
      Nages_in <- max(as.numeric(colnames(rv.Age$data[, 9:ncol(rv.Age$data)])))
      if (!is.na(Nages) & Nages_in > Nages) {
        Nages <- Nages_in
      }
      if (is.na(Nages)) {
        Nages <- Nages_in
  ...
  M_f_in <- reactive({
    ...
    if (!is.na(input$M_f_fix)) {
      M_f_in <- input$M_f_fix
    }
    if (!is.na(input$M_f_mean)) {
      M_f_in <- input$M_f_mean
    }
    if (!is.na(input$M_f_mean_sss)) {
      M_f_in <- input$M_f_mean_sss
    }
    M_f_in
  })
  ...
  M_m_in <- reactive({
    ...
    if (
      all(c(
        is.null(input$M_m),
        is.null(input$M_m_fix),
        is.null(input$M_m_mean),
        is.null(input$M_m_mean_sss)
      ))
    ) {
      return(NULL)
    }
    if (any(input$male_parms & !is.na(input$M_m))) {
      M_m_in <- input$M_m
    }
    if (any(input$male_parms_fix & !is.na(input$M_m_fix))) {
      M_m_in <- input$M_m_fix
    }
    if (any(input$male_parms_est & !is.na(input$M_m_mean))) {
      M_m_in <- input$M_m_mean
    }
    if (any(input$male_parms_SSS & !is.na(input$M_m_mean_sss))) {
      M_m_in <- input$M_m_mean_sss
    }
    M_m_in
  })
  ...
  Linf <- reactive({
    ...
    if (
      all(c(
        is.null(input$Linf_f),
        is.null(input$Linf_f_fix),
        is.null(input$Linf_f_mean),
        is.null(input$Linf_f_mean_sss)
      ))
    ) {
      return(NULL)
    }
    if (!is.na(input$Linf_f)) {
      Linf <- input$Linf_f
    }
    if (!is.na(input$Linf_f_fix)) {
      Linf <- input$Linf_f_fix
    }
    if (!is.na(input$Linf_f_mean)) {
      Linf <- input$Linf_f_mean
    }
    if (!is.na(input$Linf_f_mean_sss)) {
      Linf <- input$Linf_f_mean_sss
    }
    Linf
  })
  ...
  Linf_m_in <- reactive({
    ...
    if (any(input$male_parms & !is.na(input$Linf_m))) {
      Linf_m_in <- input$Linf_m
    }
    if (any(input$male_parms_fix & !is.na(input$Linf_m_fix))) {
      Linf_m_in <- input$Linf_m_fix
    }
    if (any(input$male_parms_est & !is.na(input$Linf_m_mean))) {
      Linf_m_in <- input$Linf_m_mean
    }
    if (any(input$male_parms_SSS & !is.na(input$Linf_m_mean_sss))) {
      Linf_m_in <- input$Linf_m_mean_sss
    }
    Linf_m_in
  })
  ...
  k_vbgf <- reactive({
    ...
    if (
      all(c(
        is.null(input$k_f),
        is.null(input$k_f_fix),
        is.null(input$k_f_mean),
        is.null(input$k_f_mean_sss)
      ))
    ) {
      return(NULL)
    }
    ...
    if (!is.na(input$k_f_fix)) {
      k_vbgf <- input$k_f_fix
    }
    if (!is.na(input$k_f_mean)) {
      k_vbgf <- input$k_f_mean
    }
    if (!is.na(input$k_f_mean_sss)) {
      k_vbgf <- input$k_f_mean_sss
    }
    k_vbgf
  })
  ...
  k_vbgf_m_in <- reactive({
    ...
    if (
      all(c(
        is.null(input$k_m),
        is.null(input$k_m_fix),
        is.null(input$k_m_mean),
        is.null(input$k_m_mean_sss)
      ))
    ) {
      return(NULL)
    }
    if (any(input$male_parms & !is.na(input$k_m))) {
      k_vbgf_m_in <- input$k_m
    }
    if (any(input$male_parms_fix & !is.na(input$k_m_fix))) {
      k_vbgf_m_in <- input$k_m_fix
    }
    if (any(input$male_parms_est & !is.na(input$k_m_mean))) {
      k_vbgf_m_in <- input$k_m_mean
    }
    if (any(input$male_parms_SSS & !is.na(input$k_m_mean_sss))) {
      k_vbgf_m_in <- input$k_m_mean_sss
    }
    k_vbgf_m_in
  })
  ...
  t0_vbgf <- reactive({
    ...
    if (
      all(c(
        is.null(input$t0_f),
        is.null(input$t0_f_fix),
        is.null(input$t0_f_mean),
        is.null(input$t0_f_mean_sss)
      ))
    ) {
      return(NULL)
    }
    ...
    if (!is.na(input$t0_f_fix)) {
      t0_vbgf <- input$t0_f_fix
    }
    if (!is.na(input$t0_f_mean)) {
      t0_vbgf <- input$t0_f_mean
    }
    if (!is.na(input$t0_f_mean_sss)) {
      t0_vbgf <- input$t0_f_mean_sss
    }
    t0_vbgf
  })
  ...
  t0_vbgf_m_in <- reactive({
    ...
    if (
      all(c(
        is.null(input$t0_m),
        is.null(input$t0_m_fix),
        is.null(input$t0_m_mean),
        is.null(input$t0_m_mean_sss)
      ))
    ) {
      return(NULL)
    }
    if (any(input$male_parms & !is.na(input$t0_m))) {
      t0_vbgf_m_in <- input$t0_m
    }
    if (any(input$male_parms_fix & !is.na(input$t0_m_fix))) {
      t0_vbgf_m_in <- input$t0_m_fix
    }
    if (any(input$male_parms_est & !is.na(input$t0_m_mean))) {
      t0_vbgf_m_in <- input$t0_m_mean
    }
    if (any(input$male_parms_SSS & !is.na(input$t0_m_mean_sss))) {
      t0_vbgf_m_in <- input$t0_m_mean_sss
    }
    t0_vbgf_m_in
  })
  ...
  L50 <- reactive({
    ...
    if (
      all(c(
        is.null(input$L50_f),
        is.null(input$L50_f_fix),
        is.null(input$L50_f_est),
        is.null(input$L50_f_sss)
      ))
    ) {
      return(NULL)
    }
    if (!is.na(input$L50_f)) {
      L50 <- input$L50_f
    }
    if (!is.na(input$L50_f_fix)) {
      L50 <- input$L50_f_fix
    }
    if (!is.na(input$L50_f_est)) {
      L50 <- input$L50_f_est
    }
    if (!is.na(input$L50_f_sss)) {
      L50 <- input$L50_f_sss
    }
    L50
  })
  ...
  L95 <- reactive({
    ...
    if (
      all(c(
        is.null(input$L95_f),
        is.null(input$L95_f_fix),
        is.null(input$L95_f_est),
        is.null(input$L95_f_sss)
      ))
    ) {
      return(NULL)
    }
    if (!is.na(input$L95_f)) {
      L95 <- input$L95_f
    }
    if (!is.na(input$L95_f_fix)) {
      L95 <- input$L95_f_fix
    }
    if (!is.na(input$L95_f_est)) {
      L95 <- input$L95_f_est
    }
  ...
  LatAmax <- reactive({
    ...
    if (
      ...
      LatAmax <- VBGF(Linf(), k_vbgf(), t0_vbgf(), 5.4 / M_f_in())
    }
    LatAmax
  })
  ...
  lt.bins.sels <- reactive({
    ...
    Sel_mode.out <- mapply(
      function(x) {
        lt.bins.out[
          as.numeric(lt.bins.freq[x, ]) == max(as.numeric(lt.bins.freq[x, ]))
        ][1]
      },
      x = 1:nrow(lt.bins.freq)
    )
    Sel_init.out <- mapply(
      function(x) lt.bins.out[as.numeric(lt.bins.freq[x, ]) > 0][1],
      x = 1:nrow(lt.bins.freq)
    )
    Sel_max.out <- mapply(
      function(x) max(lt.bins.out[lt.bins.freq[x, ] > 0]),
      x = 1:nrow(lt.bins.freq)
    )
  ...
  observeEvent(req(!is.null(rv.Ct$data)), {
    shinyjs::show(
      output$catch_plots_label <- renderText({
        "Removal history"
      })
    )
  })
  ...
  observeEvent(req(!is.null(rv.Lt$data)), {
    shinyjs::show(
      output$lt_comp_plots_label <- renderText({
        "Length compositions. Sex categories: unknown sex = 0, females = 1 and males = 2."
      })
    )
  })
    ...
    output$Ltplot_it <- renderUI({
      if (!is.null(rv.Lt$data)) {
        output$Ltplot <- renderPlot({
          ...
          Lt.dat.plot.Linf <- data.frame(
            year = 1,
            expand.grid(unique(Lt.dat.plot$fleet), unique(Lt.dat.plot$sex)),
            name = 00,
            value = 00,
            Year = 1,
            vline = -100,
          ...
          colnames(Lt.dat.plot.L50) <- colnames(Lt.dat.plot.Linf) <- colnames(
            Lt.dat.plot.LAmax
          ) <- c(
            "year",
            "fleet",
          ...
          if (!is.na(Linf())) {
            Lt.dat.plot.Linf$vline[
              Lt.dat.plot.Linf$sex == 0 | Lt.dat.plot.Linf$sex == 1
            ] <- Linf()
          }
          if (!is.na(Linf())) {
            Lt.dat.plot.Linf$vline[Lt.dat.plot.Linf$sex == 2] <- Linf_m_in()
          }
          if (!is.na(L50())) {
            Lt.dat.plot.L50$vline[
              Lt.dat.plot.L50$sex == 0 | Lt.dat.plot.L50$sex == 1
            ] <- L50()
          }
          if (!is.na(LatAmax())) {
            Lt.dat.plot.LAmax$vline[
  ...
  observeEvent(req(!is.null(rv.Age$data)), {
    shinyjs::show(
      output$marginal_age_comp_plots_label <- renderText({
        "Marginal age compositions. Sex categories: unknown sex = 0, females = 1 and males = 2."
      })
    )
  })
  ...
  observeEvent(req(!is.null(rv.Age$data)), {
    shinyjs::show(
      output$conditional_age_comp_plots_label <- renderText({
        "Conditional age at length. Sex categories: unknown sex = 0, females = 1 and males = 2."
      })
    )
  })
  ...
  observeEvent(req(!is.null(rv.Index$data)), {
    shinyjs::show(
      output$index_plots_label <- renderText({
  ...
  output$Mplot <- renderPlot({
    ...
    Nage_4_plot <- grobTree(textGrob(
      paste0("Max age =", Nages()),
      x = 0.1,
      y = 0.95,
      hjust = 0,
    ...
    if (
      ...
      output$Depletion_plot <- renderPlot({
        if (!is.na(input$status_year) & !is.na(input$Depl_mean_sss)) {
          if (input$Depl_prior_sss == "beta") {
            dep.hist.sss <- data.frame(
              draws = rbeta.ab(
                100000,
                input$Depl_mean_sss,
                input$Depl_SD_sss,
          ...
          if (input$Depl_prior_sss == "lognormal") {
            dep.hist.sss <- data.frame(
              draws = rlnorm(
                100000,
                log(input$Depl_mean_sss),
                input$Depl_SD_sss
              )
            )
          }
          if (input$Depl_prior_sss == "truncated normal") {
            dep.hist.sss <- data.frame(
              draws = rtruncnorm(
                100000,
                0,
          ...
          if (input$Depl_prior_sss == "uniform") {
            dep.hist.sss <- data.frame(
              draws = runif(100000, input$Depl_mean_sss, input$Depl_SD_sss)
            )
          }
          if (input$Depl_prior_sss == "no prior") {
            NULL
          }
          Depletion_plot <- gghistogram(
            dep.hist.sss,
            x = "draws",
            fill = "purple"
          )
    ...
    if (!is.null(input$Sel50) & !is.null(input$Selpeak)) {
        ...
        if (
          ...
          if (length(Sel50) > 1) {
            for (ii in 2:length(Sel50)) {
              Sel.out.temp <- doubleNorm24.sel(
                Sel50 = Sel50[ii],
                Selpeak = Selpeak[ii],
                PeakDesc = PeakDesc[ii],
                LtPeakFinal = LtPeakFinal[ii],
                FinalSel = FinalSel[ii]
              )
              #        Sel.out.temp<-data.frame(Bin=Sel.out.temp[,1],Sel=Sel.out.temp[,2],Fleet=fleetnames.ct[ii])
              Sel.out.temp <- data.frame(
                Bin = Sel.out.temp[, 1],
                Sel = Sel.out.temp[, 2],
                Fleet = paste0("Fleet ", ii)
              )
      ...
      if (input$Sel_choice == "Dome-shaped") {
        if (
          ...
          Selpeak <- as.numeric(trimws(unlist(strsplit(input$Selpeak, ","))))
          PeakDesc <- as.numeric(trimws(unlist(strsplit(input$PeakDesc, ","))))
          LtPeakFinal <- as.numeric(trimws(unlist(strsplit(
            input$LtPeakFinal,
            ","
          ))))
          FinalSel <- as.numeric(trimws(unlist(strsplit(input$FinalSel, ","))))
    ...
    if (!is.null(input$Sel50_sss) & !is.null(input$Selpeak_sss)) {
      ...
      if (input$Sel_choice_sss == "Logistic") {
        if (
          ...
          Selpeak <- as.numeric(trimws(unlist(strsplit(
            input$Selpeak_sss,
            ","
          ))))
          PeakDesc <- rep(10000, length(Selpeak))
      ...
      if (input$Sel_choice_sss == "Dome-shaped") {
        if (
          ...
          PeakDesc <- as.numeric(trimws(unlist(strsplit(
            input$PeakDesc_sss,
            ","
          ))))
          LtPeakFinal <- as.numeric(trimws(unlist(strsplit(
          ...
          FinalSel <- as.numeric(trimws(unlist(strsplit(
            input$FinalSel_sss,
            ","
          ))))
  ...
  SSS.run <- observeEvent(input$run_SSS, {
    ...
    {
      file.copy(
        paste0("SSS_files/sssexample_BH"),
        paste0("Scenarios"),
        recursive = TRUE,
        overwrite = TRUE
      )
      file.rename(
        paste0("Scenarios/sssexample_BH"),
    ...
    data.file$Comments <- c(
      data.file$Comments,
      input$scenario_description_input
    )
    ...
    if (!is.null(rv.Index$data)) {
      ...
      if (index.fleets > catch.dep.fleets) {
        data.file$Nfleets <- index.fleets
      }
      if (index.fleets == catch.dep.fleets) {
        data.file$Nfleets <- index.fleets + 1
      }
      if (index.fleets < catch.dep.fleets) {
        data.file$Nfleets <- catch.dep.fleets
      }
    ...
    if ((data.file$Nfleets - 1) > 1) {
      for (i in 1:(data.file$Nfleets - 2)) {
        data.file$fleetinfo <- rbind(
          data.file$fleetinfo,
          data.file$fleetinfo[1, ]
        )
        data.file$CPUEinfo <- rbind(data.file$CPUEinfo, data.file$CPUEinfo[1, ])
      }
      data.file$fleetinfo$fleetname <- c(
        paste0("Fishery", 1:(catch.dep.fleets - 1)),
        "Depl"
      )
    ...
    data.file$maximum_size <- ceiling(
      input$Linf_f_mean_sss + (input$Linf_f_mean_sss * 0.1)
    )
    ...
    #Females
    #M
    if (input$M_f_prior == "lognormal") {
      ctl.file$MG_parms[1, 3:4] <- c(
        input$M_f_mean_sss,
        log(input$M_f_mean_sss)
      )
    } else {
    ...
    if (input$t0_f_prior == "lognormal") {
      ctl.file$MG_parms[2, 3:4] <- c(0, log(0.0000001))
    } else {
    ...
    if (input$CV_lt_f_young_prior == "lognormal") {
      ctl.file$MG_parms[5, 3:4] <- c(
        input$CV_lt_f_young_mean_sss,
        log(input$CV_lt_f_young_mean_sss)
      )
    } else {
    ...
    if (input$male_parms_SSS) {
      ...
      if (input$t0_f_prior_sss == "lognormal") {
        ...
        ctl.file$MG_parms[14, 3:4] <- c(male_vbgf_sss[1], male_vbgf_sss[1])
      }
      ...
      if (input$CV_lt_f_young_prior_sss == "lognormal") {
        ctl.file$MG_parms[17, 3:4] <- c(
          ...
          log(input$CV_lt_m_young_mean_sss)
        )
      } else {
        ctl.file$MG_parms[17, 3:4] <- c(
          input$CV_lt_m_young_mean_sss,
      ...
      if (input$CV_lt_f_old_prior_sss == "lognormal") {
        ctl.file$MG_parms[18, 3:4] <- c(
          ...
          log(input$CV_lt_m_old_mean_sss)
        )
      } else {
        ctl.file$MG_parms[18, 3:4] <- c(
          input$CV_lt_m_old_mean_sss,
    ...
    if (input$h_ss_prior == "lognormal") {
      ...
      ctl.file$SR_parms[2, 3:4] <- input$h_mean_ss
    }
    #}
    ...
    if (input$Sel_choice_sss == "Logistic") {
      ...
      ctl.file$size_selex_parms[1, 1:2] <- c(
        ...
        max(data.file$lbin_vector)
      )
      ctl.file$size_selex_parms[1, 3:4] <- Selpeak[1]
      ctl.file$size_selex_parms[2, 3:4] <- 15
      ctl.file$size_selex_parms[3, 3:4] <- log(
        -((Sel50[1] - Selpeak[1])^2 / log(0.5))
      )
    ...
    if (input$Sel_choice_sss == "Dome-shaped") {
      ...
      ctl.file$size_selex_parms[1, 3:4] <- Selpeak[1]
      ctl.file$size_selex_parms[2, 3:4] <- -log(
        (max(data.file$lbin_vector) - Selpeak[1] - bin.width) /
          (PeakDesc[1] - Selpeak[1] - bin.width + 0.000000001)
      )
      ctl.file$size_selex_parms[3, 3:4] <- log(
        -((Sel50[1] - Selpeak[1])^2 / log(0.5))
      )
      ctl.file$size_selex_parms[4, 3:4] <- log(LtPeakFinal[1])
      ctl.file$size_selex_parms[6, 3:4] <- -log(
    ...
    if ((data.file$Nfleets - 1) > 1) {
      for (i in 1:(data.file$Nfleets - 2)) {
        ...
        ctl.file$size_selex_types <- rbind(
          ctl.file$size_selex_types,
          ctl.file$size_selex_types[1, ]
        )
        ctl.file$age_selex_types <- rbind(
          ctl.file$age_selex_types,
          ctl.file$age_selex_types[1, ]
        )
        ctl.file$size_selex_parms <- rbind(
        ...
        if (input$Sel_choice_sss == "Dome-shaped") {
          ...
          ctl.file$size_selex_parms[6 * i + 2, 3:4] <- -log(
            (max(data.file$lbin_vector) - Selpeak[i + 1] - bin.width) /
              (PeakDesc[i + 1] - Selpeak[i + 1] - bin.width + 0.000000001)
          )
          ctl.file$size_selex_parms[6 * i + 3, 3:4] <- log(
            -((Sel50[i + 1] - Selpeak[i + 1])^2 / log(0.5))
          )
          ctl.file$size_selex_parms[6 * i + 4, 3:4] <- log(LtPeakFinal[i + 1])
          ctl.file$size_selex_parms[6 * i + 6, 3:4] <- -log(
            (1 / (FinalSel[i + 1] + 0.000000001) - 1)
          )
        }
      }
      ...
      rownames(ctl.file$age_selex_types) <- rownames(
        ctl.file$size_selex_types
      ) <- c(paste0("Fishery", 1:(data.file$Nfleets - 1)), "Depl")
      size_selex_parms_rownames <- list()
      for (f_i in 1:(data.file$Nfleets - 1)) {
        size_selex_parms_rownames[[f_i]] <- c(
          paste0("SizeSel_P_1_Fishery", f_i, "(", f_i, ")"),
    ...
    if (input$RP_choices) {
      ...
      forecast.file$ControlRuleMethod <- CR_choices_num.vec[
        CR_choices == input$CR_Ct_F
      ]
    ...
    if (input$Forecast_choice) {
      ...
      buffer.in <- as.numeric(trimws(unlist(strsplit(
        input$forecast_buffer,
        ","
      ))))
      if (length(buffer.in) == 1) {
        forecast.file$Flimitfraction <- buffer.in
      }
      if (length(buffer.in) > 1) {
        forecast.file$Flimitfraction <- -1
        buffer.datafr <- data.frame(
          year = c(
            (data.file$endyr + 1):(data.file$endyr + input$forecast_num)
          ),
          fraction = buffer.in
        )
        rownames(buffer.datafr) <- paste0(
          "#_Flimitfraction_m",
          1:input$forecast_num
        )
    ...
    if (!input$male_offset_SSS) {
      ...
      Linf.in_sss <- c(
        sss.prior.type[sss.prior.name == input$Linf_f_prior_sss],
        input$Linf_f_mean_sss,
        input$Linf_f_SD_sss,
        sss.prior.type[sss.prior.name == input$Linf_f_prior_sss],
        input$Linf_f_mean_sss,
      ...
      k.in_sss <- c(
        sss.prior.type[sss.prior.name == input$k_f_prior_sss],
        input$k_f_mean_sss,
        input$k_f_SD_sss,
        sss.prior.type[sss.prior.name == input$k_f_prior_sss],
        input$k_f_mean_sss,
      ...
      t0.in_sss <- c(
        sss.prior.type[sss.prior.name == input$t0_f_prior_sss],
        input$t0_f_mean_sss,
        input$t0_f_SD_sss,
        sss.prior.type[sss.prior.name == input$t0_f_prior_sss],
        input$t0_f_mean_sss,
    ...
    if (input$male_parms_SSS) {
      M.in_sss <- c(
        ...
        input$M_f_mean_sss,
        input$M_f_SD_sss,
        sss.prior.type[sss.prior.name == input$M_m_prior_sss],
        input$M_m_mean_sss,
        input$M_m_SD_sss
      )
      Linf.in_sss <- c(
        ...
        input$Linf_f_mean_sss,
        input$Linf_f_SD_sss,
        sss.prior.type[sss.prior.name == input$Linf_m_prior_sss],
        input$Linf_m_mean_sss,
        input$Linf_f_SD_sss
      )
      k.in_sss <- c(
        ...
        input$k_f_mean_sss,
        input$k_f_SD_sss,
        sss.prior.type[sss.prior.name == input$k_m_prior_sss],
        input$k_m_mean_sss,
        input$k_m_SD_sss
      )
      t0.in_sss <- c(
        ...
        input$t0_f_mean_sss,
        input$t0_f_SD_sss,
        sss.prior.type[sss.prior.name == input$t0_m_prior_sss],
        input$t0_m_mean_sss,
        input$t0_m_SD_sss
      )
    }
    ...
    if (is.na(input$Depl_mean_sss) | input$Depl_mean_sss < 0) {
      ...
      SSS.out <- SSS(
        ...
        doR0.loop = c(
          1,
          round(input$lnR0_sss * 0.5),
          round(input$lnR0_sss * 1.5),
          (round(input$lnR0_sss * 1.3) - round(input$lnR0_sss * 0.5)) / 10
        ),
        sum_age = 0,
      ...
      if (
        ...
        output$SSS_priors_post <- renderPlot({
          if (
            ...
            load(paste0("Scenarios/", input$Scenario_name, "/SSS_out.rds"))
            sss.M.f <- rbind(
              data.frame(
                value = SSS.out$Prior$M_f,
                type = "prior",
                metric = "Female M"
              ),
              data.frame(
                value = SSS.out$Post$M_f,
                type = "post",
                metric = "Female M"
              )
            )
            sss.M.m <- rbind(
              data.frame(
                value = SSS.out$Prior$M_m,
                type = "prior",
                metric = "Male M"
              ),
              data.frame(
                value = SSS.out$Post$M_m,
                type = "post",
                metric = "Male M"
              )
            )
            sss.h <- rbind(
              data.frame(value = SSS.out$Prior$h, type = "prior", metric = "h"),
              data.frame(value = SSS.out$Post$h, type = "post", metric = "h")
            )
            sss.Dep <- rbind(
              data.frame(
                value = SSS.out$Prior$Dep,
                type = "prior",
                metric = "Dep"
              ),
              data.frame(
                value = SSS.out$Post$Dep.Obs,
                type = "post",
                metric = "Dep"
              )
            )
        ...
        output$SSS_growth_priors_post <- renderPlot({
          if (
            ...
            sss.L1_f <- rbind(
              data.frame(
                value = SSS.out$Prior$L1_f,
                type = "prior",
                metric = "Female L1"
              ),
              data.frame(
                value = SSS.out$Post$L1_f,
                type = "post",
                metric = "Female L1"
              )
            )
            sss.Linf_f <- rbind(
              data.frame(
                value = SSS.out$Prior$Linf_f,
                type = "prior",
                metric = "Female Linf"
              ),
              data.frame(
                value = SSS.out$Post$Linf_f,
                type = "post",
                metric = "Female Linf"
              )
            )
            sss.k_f <- rbind(
              data.frame(
                value = SSS.out$Prior$k_f,
                type = "prior",
                metric = "Female k"
              ),
              data.frame(
                value = SSS.out$Post$k_f,
                type = "post",
                metric = "Female k"
              )
            )
            sss.L1_m <- rbind(
              data.frame(
                value = SSS.out$Prior$L1_m,
                type = "prior",
                metric = "Male L1"
              ),
              data.frame(
                value = SSS.out$Post$L1_m,
                type = "post",
                metric = "Male L1"
              )
            )
            sss.Linf_m <- rbind(
              data.frame(
                value = SSS.out$Prior$Linf_m,
                type = "prior",
                metric = "Male Linf"
              ),
              data.frame(
                value = SSS.out$Post$Linf_m,
                type = "post",
                metric = "Male Linf"
              )
            )
            sss.k_m <- rbind(
              data.frame(
                value = SSS.out$Prior$k_m,
                type = "prior",
                metric = "Male k"
              ),
              data.frame(
                value = SSS.out$Post$k_m,
                type = "post",
                metric = "Male k"
              )
            )
            sss.vals.growth.out <- rbind(
              sss.L1_f,
              sss.Linf_f,
              sss.k_f,
              sss.L1_m,
        ...
        output$SSS_OFL_plot <- renderPlot({
          if (
            ...
            ggplot(
              reshape2::melt(SSS.out$OFL),
              aes(Var2, value, group = Var2)
            ) +
              geom_boxplot(fill = "#236192") +
              scale_x_continuous(
                breaks = ofl.years,
                labels = as.character(ofl.years)
              ) +
              ylab("OFL (mt)") +
              xlab("Year")
          } else {
        ...
        output$SSS_ABC_plot <- renderPlot({
          if (
            ...
            ggplot(
              reshape2::melt(SSS.out$ABC),
              aes(Var2, value, group = Var2)
            ) +
              geom_boxplot(fill = "#658D1B") +
              scale_x_continuous(
                breaks = abc.years,
                labels = as.character(abc.years)
              ) +
              ylab("ABC (mt)") +
              xlab("Year")
          } else {
  ...
  SS.file.update <- observeEvent(input$run_SS, {
    ...
    if (!input$user_model) {
      ...
      ctl.file <- SS_readctl(
        paste0("Scenarios/", input$Scenario_name, "/controlfile.ctl"),
        use_datlist = TRUE,
        datlist = data.file
      )
    }
    ...
    if (!input$user_model) {
      ...
      Sel50 <- as.numeric(trimws(unlist(strsplit(input$Sel50, ","))))
      Sel50_phase <- as.numeric(trimws(unlist(strsplit(
        input$Sel50_phase,
        ","
      ))))
      Selpeak <- as.numeric(trimws(unlist(strsplit(input$Selpeak, ","))))
      Selpeak_phase <- as.numeric(trimws(unlist(strsplit(
        input$Selpeak_phase,
        ","
      ))))
      bin.width <- data.file$lbin_vector[2] - data.file$lbin_vector[1]
      ...
      if (input$Sel_choice == "Dome-shaped") {
        ...
        LtPeakFinal_phase <- as.numeric(trimws(unlist(strsplit(
          input$LtPeakFinal_phase,
          ","
        ))))
        FinalSel <- as.numeric(trimws(unlist(strsplit(input$FinalSel, ","))))
        FinalSel_phase <- as.numeric(trimws(unlist(strsplit(
          input$FinalSel_phase,
          ","
        ))))
        minmaxbin <- c(
          Selpeak[1] - min(data.file$lbin_vector),
        ...
        sel.inputs.lts <- c(
          ...
          length(Selpeak_phase),
          length(PeakDesc),
          length(PeakDesc_phase),
          length(LtPeakFinal),
          length(LtPeakFinal_phase),
          length(FinalSel),
      ...
      if (
        ...
        if (is.null(rv.Ct$data)) {
          data.file$spawn_month <- input$rec_month_LO
        }
        if (!is.null(rv.Ct$data) & input$est_parms == F) {
          data.file$spawn_month <- input$rec_month
        }
        if (!is.null(rv.Ct$data) & input$est_parms == T) {
          data.file$spawn_month <- input$rec_month_est
        }
        catch.fleets.Ct <- catch.fleets.Lt <- catch.fleets.Age <- NA
        if (!is.null(rv.Ct$data)) {
          catch.fleets.Ct <- max(ncol(rv.Ct$data) - 1)
        }
        if (all(!is.null(rv.Lt$data), is.null(rv.Ct$data))) {
          catch.fleets.Lt <- max(rv.Lt$data[, 3])
        }
        if (all(!is.null(rv.Age$data), is.null(rv.Ct$data))) {
          catch.fleets.Age <- max(rv.Age$data[, 3])
        }
        catch.fleets <- max(
          catch.fleets.Ct,
          catch.fleets.Lt,
        ...
        if (is.null(rv.Ct$data)) {
          ...
          if (input$Ct_F_LO_select == "Estimate F") {
            data.file$bycatch_fleet_info[4:5] <- c(input$styr, input$endyr)
          }
          if (data.file$Nfleets > 1) {
            for (i in 1:(data.file$Nfleets - 1)) {
              if (input$Ct_F_LO_select == "Estimate F") {
                data.file$bycatch_fleet_info <- rbind(
                  data.file$bycatch_fleet_info,
                  data.file$bycatch_fleet_info[1, ]
                )
              }
            }
            if (input$Ct_F_LO_select == "Estimate F") {
          ...
          if (catch.fleets > 1) {
            catch.level <- as.numeric(trimws(unlist(strsplit(
              input$Wt_fleet_Ct,
              ","
            ))))
        ...
        if (!is.null(rv.Ct$data)) {
          ...
          data.file$catch$catch[
            data.file$catch$year == -999
          ] <- as.numeric(trimws(unlist(strsplit(input$Eq_Ct_fleet, ","))))
          data.file$catch$catch[
        ...
        if (!is.null(rv.Index$data)) {
          ...
          data.file$CPUE <- data.frame(
            year = rv.Index$data[, 1],
            month = rv.Index$data[, 2],
            index = rv.Index$data[, 3],
            obs = rv.Index$data[, 4],
        ...
        data.file$binwidth <- 2
        if (!is.null(rv.Lt$data)) {
          data.file$binwidth <- as.numeric(colnames(rv.Lt$data)[7]) -
            as.numeric(colnames(rv.Lt$data)[6])
        }
        ...
        if (!is.null(rv.Lt$data)) {
          ...
          data.file$lbin_vector <- as.numeric(colnames(rv.Lt$data)[
            6:ncol(rv.Lt$data)
          ...
          lt.data.names <- c(
            colnames(data.file$lencomp[, 1:6]),
            paste0("f", data.file$lbin_vector),
            paste0("m", data.file$lbin_vector)
          )
          lt.data.females <- lt.data.males <- lt.data.unknowns <- lt.data.sex3 <- data.frame(matrix(
            rep(NA, length(lt.data.names)),
            nrow = 1
          ))
          colnames(Lt.comp.data)[1:5] <- c(
            "Year",
            "Month",
            "Fleet",
            "Sex",
          ...
          if (nrow(subset(Lt.comp.data, Sex == 1)) > 0) {
            ...
            lt.data.females <- data.frame(cbind(
              samp.yrs,
          ...
          if (nrow(subset(Lt.comp.data, Sex == 2)) > 0) {
            ...
            lt.data.males <- data.frame(cbind(
              samp.yrs_males,
              Lt.comp.data_male[, 2],
              Lt.comp.data_male[, 3],
              Lt.comp.data_male[, 4],
          ...
          if (nrow(subset(Lt.comp.data, Sex == 0)) > 0) {
            ...
            lt.data.unknowns <- data.frame(cbind(
              samp.yrs_unknown,
          ...
          if (input$Sex3options) {
            if (input$Sex3) {
              yrsfleet_females <- paste0(
                Lt.comp.data_female[, 1],
                Lt.comp.data_female[, 3]
              )
              yrsfleet_males <- paste0(
                Lt.comp.data_male[, 1],
                Lt.comp.data_male[, 3]
              )
              #Match years
              ...
              #Subset years
              Lt.comp.data_female_sex3 <- Lt.comp.data_female[
                sex3_match_female,
              ]
              Lt.comp.data_male_sex3 <- Lt.comp.data_male[sex3_match_male, ]
              lt.data.sex3 <- data.frame(cbind(
                Lt.comp.data_female_sex3[, 1],
        ...
        if (!is.null(Age.comp.data)) {
          ...
          age.data.names <- c(
            ...
            paste0("f", data.file$agebin_vector),
            paste0("m", data.file$agebin_vector)
          )
          age.data.females <- age.data.males <- age.data.unknowns <- age.data.sex3 <- data.frame(matrix(
            rep(NA, length(age.data.names)),
            nrow = 1
          ))
          colnames(Age.comp.data)[1:8] <- c(
            "Year",
            "Month",
            "Fleet",
            "Sex",
          ...
          if (nrow(subset(Age.comp.data, Sex == 1)) > 0) {
            ...
            age.data.females <- data.frame(cbind(
              samp.yrs_females,
          ...
          if (nrow(subset(Age.comp.data, Sex == 2)) > 0) {
            ...
            age.data.males <- data.frame(cbind(
              samp.yrs_males,
          ...
          if (nrow(subset(Age.comp.data, Sex == 0)) > 0) {
            Age.comp.data_unknown <- subset(
              Age.comp.data,
              Sex == 0 & Nsamps > 0
            )
            samp.yrs_unknown <- Age.comp.data_unknown[, 1]
            age.data.unknowns <- data.frame(cbind(
              samp.yrs_unknown,
          ...
          if (input$Sex3options) {
            if (input$AgeSex3) {
              age_yrsfleetagetype_females <- paste0(
                Age.comp.data_female[, 1],
                Age.comp.data_female[, 3],
                Age.comp.data_female[, 6]
              )
              age_yrsfleetagetype_males <- paste0(
                Age.comp.data_male[, 1],
                Age.comp.data_male[, 3],
                Age.comp.data_male[, 6]
              )
              #Match years
              age_sex3_match_female <- age_yrsfleetagetype_females %in%
                age_yrsfleetagetype_males
              age_sex3_match_male <- age_yrsfleetagetype_males %in%
                age_yrsfleetagetype_females
              #Subset years
              Age.comp.data_female_sex3 <- Age.comp.data_female[
                age_sex3_match_female,
              ]
              Age.comp.data_male_sex3 <- Age.comp.data_male[
                age_sex3_match_male,
              ]
              age.data.sex3 <- data.frame(cbind(
          ...
          #  }
          colnames(age.data.females) <- colnames(age.data.males) <- colnames(
            age.data.unknowns
          ) <- colnames(age.data.sex3) <- age.data.names
          data.file$agecomp <- na.omit(rbind(
            age.data.females,
            age.data.males,
            age.data.unknowns,
            age.data.sex3
        ...
        if (data.file$Nfleets > 1) {
          for (i in 1:(data.file$Nfleets - 1)) {
            ...
            data.file$CPUEinfo <- rbind(
              data.file$CPUEinfo,
              data.file$CPUEinfo[1, ]
            )
            data.file$len_info <- rbind(
              data.file$len_info,
              data.file$len_info[1, ]
            )
            data.file$age_info <- rbind(
              data.file$age_info,
          ...
          if (!is.null(rv.Ct$data)) {
            ...
            if (length(data.file$fleetinfo$fleetname) > length(fishery.names)) {
              data.file$fleetinfo[
                c(
                  (length(fishery.names) + 1):length(
                    data.file$fleetinfo$fleetname
                  )
                ),
                1
        ...
        SS_writedat(
          data.file,
          paste0("Scenarios/", input$Scenario_name, "/datafile.dat"),
          overwrite = TRUE
        )
        #}
        ####################### END DATA FILE #####################################
        ...
        if (
          ...
          ctl.file$MG_parms[5, 3:4] <- as.numeric(trimws(unlist(strsplit(
            input$CV_lt_f,
            ","
          ))))[1] #CV
          ctl.file$MG_parms[6, 3:4] <- as.numeric(trimws(unlist(strsplit(
            input$CV_lt_f,
            ","
          ))))[2] #CV
          #Maturity
          ...
          ctl.file$MG_parms[10, 3:4] <- log(0.05 / 0.95) /
            (input$L95_f - input$L50_f) #Maturity slope
          ...
          ctl.file$MG_parms[17, 3:4] <- as.numeric(trimws(unlist(strsplit(
            input$CV_lt_f,
            ","
          ))))[1] #CV
          ctl.file$MG_parms[18, 3:4] <- as.numeric(trimws(unlist(strsplit(
            input$CV_lt_f,
            ","
          ))))[2] #CV
          ...
          if (input$male_parms) {
            ...
            if (
              as.numeric(trimws(unlist(strsplit(input$CV_lt_m, ","))))[1] == 0
            ) {
            ...
            if (
              as.numeric(trimws(unlist(strsplit(input$CV_lt_m, ","))))[2] == 0
            ) {
        ...
        if (
          ...
          if (input$male_parms_fix) {
            ...
            if (input$Linf_m_fix == 0) {
              ctl.file$MG_parms[15, 3:6] <- 0
            } #Linf
            ctl.file$MG_parms[16, 3:4] <- input$k_m_fix #k
            if (input$k_m_fix == 0) {
              ctl.file$MG_parms[16, 3:4] <- 0
            } #k
            ctl.file$MG_parms[17, 3:4] <- as.numeric(trimws(unlist(strsplit(
              input$CV_lt_m_fix,
              ","
        ...
        if (
          all(
            any(input$est_parms == TRUE, input$est_parms2 == FALSE),
            any(
              all(!is.null(rv.Lt$data), !is.null(rv.Ct$data)),
              all(!is.null(rv.Age$data), !is.null(rv.Ct$data)),
              all(!is.null(rv.Index$data), !is.null(rv.Ct$data))
          ...
          #M
          if (input$M_f_prior == "lognormal") {
            ctl.file$MG_parms[1, 3:4] <- c(input$M_f_mean, log(input$M_f_mean))
          } else {
            ctl.file$MG_parms[1, 3:4] <- c(input$M_f_mean, input$M_f_mean)
          }
          ...
          if (input$Linf_f_prior == "lognormal") {
            ctl.file$MG_parms[3, 3:4] <- c(
              input$Linf_f_mean,
              log(input$Linf_f_mean)
            )
          } else {
            ctl.file$MG_parms[3, 3:4] <- input$Linf_f_mean
          }
          ctl.file$MG_parms[3, 5] <- input$Linf_f_SD
          ctl.file$MG_parms[3, 6] <- prior.type[
          ...
          if (input$k_f_prior == "lognormal") {
            ...
            ctl.file$MG_parms[4, 3:4] <- input$k_f_mean
          }
          ...
          if (input$CV_lt_f_young_prior == "lognormal") {
            ctl.file$MG_parms[5, 3:4] <- c(
              input$CV_lt_f_young_mean,
              log(input$CV_lt_f_young_mean)
            )
          } else {
            ctl.file$MG_parms[5, 3:4] <- input$CV_lt_f_young_mean
          }
          ctl.file$MG_parms[5, 5] <- input$CV_lt_f_young_SD
          ctl.file$MG_parms[5, 6] <- prior.type[
          ...
          if (input$CV_lt_f_old_prior == "lognormal") {
            ctl.file$MG_parms[6, 3:4] <- c(
              input$CV_lt_f_old_mean,
              log(input$CV_lt_f_old_mean)
            )
          } else {
          ...
          ctl.file$MG_parms[6, 6] <- prior.type[
          ...
          if (input$male_parms_est) {
            ...
            if (input$M_m_prior == "lognormal") {
              ...
              ctl.file$MG_parms[13, 3:4] <- c(input$M_m_mean, input$M_m_mean)
            }
            ctl.file$MG_parms[13, 5] <- input$M_m_SD
            ctl.file$MG_parms[13, 6] <- prior.type[
            ...
            if (input$t0_m_prior == "lognormal") {
              ctl.file$MG_parms[14, 3:4] <- c(
                male_vbgf_est[1],
            ...
            ctl.file$MG_parms[14, 6] <- prior.type[
            ...
            if (input$Linf_m_prior == "lognormal") {
              ctl.file$MG_parms[15, 3:4] <- c(
                input$Linf_m_mean,
                log(input$Linf_m_mean)
              )
            } else {
              ctl.file$MG_parms[15, 3:4] <- input$Linf_m_mean
            }
            ctl.file$MG_parms[15, 5] <- input$Linf_m_SD
            ctl.file$MG_parms[15, 6] <- prior.type[
            ...
            if (input$k_m_prior == "lognormal") {
              ...
              ctl.file$MG_parms[16, 3:4] <- input$k_m_mean
            }
            ctl.file$MG_parms[16, 5] <- input$k_m_SD
            ctl.file$MG_parms[16, 6] <- prior.type[
            ...
            if (input$CV_lt_m_young_prior == "lognormal") {
              ctl.file$MG_parms[17, 3:4] <- c(
                input$CV_lt_m_young_mean,
                log(input$CV_lt_m_young_mean)
              )
            } else {
              ctl.file$MG_parms[17, 3:4] <- input$CV_lt_m_young_mean
            }
            ctl.file$MG_parms[17, 5] <- input$CV_lt_m_young_SD
            ctl.file$MG_parms[17, 6] <- prior.type[
            ...
            if (input$CV_lt_m_old_prior == "lognormal") {
              ctl.file$MG_parms[18, 3:4] <- c(
                input$CV_lt_m_old_mean,
                log(input$CV_lt_m_old_mean)
              )
            } else {
              ctl.file$MG_parms[18, 3:4] <- input$CV_lt_m_old_mean
            }
            ctl.file$MG_parms[18, 5] <- input$CV_lt_m_old_SD
            ctl.file$MG_parms[18, 6] <- prior.type[
        ...
        if (input$rec_choice) {
          ...
          if (input$RecDevChoice == "2: Simple deviations") {
            ctl.file$do_recdev <- 2
          }
          if (input$RecDevChoice == "3: deviation vector") {
            ctl.file$do_recdev <- 3
          }
          if (input$RecDevChoice == "4: option 3 plus penalties") {
            ctl.file$do_recdev <- 4
          }
        ...
        #Length Selectivity
        if (input$Ct_F_LO_select == "Estimate F" & is.null(rv.Ct$data)) {
          ctl.file$size_selex_types[2] <- 3
        } #Change to recognize discard fishery
        Sel50 <- as.numeric(trimws(unlist(strsplit(input$Sel50, ","))))
        Sel50_phase <- as.numeric(trimws(unlist(strsplit(
          input$Sel50_phase,
          ","
        ...
        if (input$Sel_choice == "Logistic") {
          ...
          ctl.file$size_selex_parms[1, 1:2] <- c(
            Selpeak[1] - minmaxbin[1],
            Selpeak[1] + minmaxbin[2]
          )
          ctl.file$size_selex_parms[1, 3:4] <- Selpeak[1]
          ctl.file$size_selex_parms[2, 3:4] <- 15
          ctl.file$size_selex_parms[3, 3:4] <- log(
        ...
        if (data.file$Nfleets > 1) {
          for (i in 1:(data.file$Nfleets - 1)) {
            ...
            if (input$Sel_choice == "Dome-shaped") {
              ...
              ctl.file$size_selex_parms[6 * i + 3, 7] <- Sel50_phase[i + 1]
              ctl.file$size_selex_parms[6 * i + 4, 3:4] <- log(LtPeakFinal[
                i + 1
              ])
              ctl.file$size_selex_parms[6 * i + 4, 7] <- LtPeakFinal_phase[
                i + 1
              ]
              ctl.file$size_selex_parms[6 * i + 6, 3:4] <- -log(
                (1 / (FinalSel[i + 1] + 0.000000001) - 1)
          ...
          rownames(ctl.file$age_selex_types) <- rownames(
            ctl.file$size_selex_types
          ) <- paste0("Fishery", 1:data.file$Nfleets)
          size_selex_parms_rownames <- list()
        ...
        if (!is.null(rv.Index$data) | data.file$Nfleets > catch.fleets) {
          ...
          q.setup.lines <- data.frame(t(c(
            unique(rv.Index$data[, 3])[1],
            1,
            0,
            0,
            0,
          ...
          if (input$Indexvar) {
            q.lines <- data.frame(rbind(
              c(-15, 15, 1, 0, 1, 0, -1, rep(0, 7)),
              c(0, 5, 0, 0, 99, 0, 3, 0, 0, 0, 0, 0, 0, 0)
            ))
          }
          if (length(unique(rv.Index$data[, 3])) > 1) {
            for (q in 2:length(unique(rv.Index$data[, 3]))) {
              if (!input$Indexvar) {
                q.setup.lines <- rbind(
                  q.setup.lines,
                  c(unique(rv.Index$data[, 3])[q], 1, 0, 0, 0, 1)
                )
              ...
              if (input$Indexvar) {
                q.setup.lines <- rbind(
                  q.setup.lines,
                  c(unique(rv.Index$data[, 3])[q], 1, 0, 1, 0, 1)
                )
                q.lines <- rbind(
                  q.lines,
                  data.frame(rbind(
          ...
          if (!input$Indexvar) {
            rownames(q.lines) <- paste0(
              "LnQ_base_",
              unique(rv.Index$data[, 6]),
              "(",
              unique(rv.Index$data[, 3]),
              ")"
            )
          }
          ...
          if (data.file$Nfleets > catch.fleets) {
            if (any(fleet.survey.names == "RSS")) {
              ...
              ctl.file$size_selex_parms <- ctl.file$size_selex_parms[
                -c((RSS.index * 6 - 5):(RSS.index * 6)),
              ] #Remove selectivity related to RSS
            }
          }
        }
        ...
        if (is.null(rv.Ct$data)) {
          ...
          if (data.file$Nfleets > 1) {
            for (i_lam in 2:data.file$Nfleets) {
              ...
              lts.lambdas_temp[1, 2] <- ct.lambdas_temp[
                1,
                2
              ] <- init.ct.lambdas_temp[1, 2] <- i_lam
          ...
          if (input$Ct_F_LO_select == "Estimate F") {
            if (data.file$Nfleets > 1) {
              lt.lam.in <- as.numeric(trimws(unlist(strsplit(
                input$Wt_fleet_Ct,
                ","
              )))) /
                sum(as.numeric(trimws(unlist(strsplit(
                  input$Wt_fleet_Ct,
                  ","
                )))))
          ...
          rownames(lts.lambdas) <- paste0(
            "length_Fishery",
            c(1:data.file$Nfleets),
            "_sizefreq_method_1_Phz1"
          )
          ct.lambdas[, 4] <- 0
          rownames(ct.lambdas) <- paste0(
            "catch_Fishery",
            c(1:data.file$Nfleets),
            "_Phz1"
          )
          init.ct.lambdas[, 4] <- 0
          rownames(init.ct.lambdas) <- paste0(
            "init_equ_catch_Fishery",
            c(1:data.file$Nfleets),
            "_lambda_for_init_equ_catch_can_only_enable/disable for_all_fleets_Phz1"
          )
        ...
        if (!is.null(rv.Ct$data)) {
          ...
          ctl.file$init_F$INIT[
            as.numeric(trimws(unlist(strsplit(input$Eq_Ct_fleet, ",")))) >
              0.0000000000001
          ] <- 0.01
          ctl.file$init_F$PHASE[
            as.numeric(trimws(unlist(strsplit(input$Eq_Ct_fleet, ",")))) >
              0.0000000000001
          ] <- 1
          ctl.file$lambdas[
            grep("init_equ", rownames(ctl.file$lambdas)),
          ]$value[
            as.numeric(trimws(unlist(strsplit(input$Eq_Ct_fleet, ",")))) >
              0.0000000000001
          ] <- 1
    ...
    if (input$user_model) {
      ...
      if (input$scenario_description == TRUE) {
        data.file_user <- SS_readdat(file.path(
          "Scenarios",
          input$Scenario_name,
          starter.file$datfile
        ))
        data.file_user$Comments <- c(
          data.file_user$Comments,
          input$scenario_description_input
        )
        SS_writedat(
          data.file_user,
        ...
        ctl.file_user <- SS_readctl(file.path(
          "Scenarios",
          input$Scenario_name,
          starter.file$ctlfile
        ))
        ctl.file_user$Comments <- c(
          ctl.file_user$Comments,
          input$scenario_description_input
        )
        SS_writectl(
          ctl.file_user,
          file.path("Scenarios", input$Scenario_name, starter.file$ctlfile),
          overwrite = TRUE
        )
      }
    ...
    if (exists("checkmod") | input$user_model) {
      ...
      if (input$Data_wt == "Dirichlet-multinomial") {
        DataWT_opt <- "DM"
      }
      if (input$Data_wt == "Francis") {
        DataWT_opt <- "Francis"
      }
      ...
      if (
        ...
        if (input$Data_wt != "None or Current weighting") {
          if (Model.output$inputs$covar == TRUE) {
            tune_comps(
              Model.output,
              dir = paste0("Scenarios/", input$Scenario_name),
              niters_tuning = 3,
              option = DataWT_opt,
        ...
        if (is.null(input$no_plots_tables)) {
          show_modal_spinner(
            spin = "flower",
            color = wes_palettes$Zissou1[4],
            text = "Making plots"
          )
          RPs_4_plots <- as.numeric(trimws(unlist(strsplit(
        ...
        if (input$jitter_choice) {
          if (input$Njitter > 0) {
            show_modal_spinner(
              spin = "flower",
              color = wes_palettes$Moonrise1[1],
              text = "Run jitters"
            )
            ...
            minlikes <- profilesummary$likelihoods[
              1,
            ...
            jitter.likes <- profilesummary$likelihoods[
              1,
              -length(profilesummary$likelihoods)
            ]
            ...
            points(
              c(1:length(jitter.likes))[jitter.likes > ref.like],
              jitter.likes[jitter.likes > ref.like],
              type = "p",
              col = "black",
              bg = "red",
              pch = 21,
            ...
            dev.off()
            save(profilesummary, file = paste0("jitter_summary.rds"))
            SSplotComparisons(
              profilesummary,
              legendlabels = c(0:input$Njitter),
              ylimAdj = 1.30,
              subplot = c(1),
              new = FALSE,
              print = TRUE,
            ...
            show_modal_spinner(
              spin = "flower",
              color = wes_palettes$Moonrise1[2],
              text = "Re-run best model post-jitters"
            )
            file.copy(
              paste0(
                main.dir,
              ...
              paste0(main.dir, "/Scenarios/", input$Scenario_name, "/ss3.par"),
              overwrite = TRUE
            )
            ...
            SS_writestarter(
              starter.file,
              paste0(main.dir, "/Scenarios/", input$Scenario_name),
              overwrite = TRUE
            )
            RUN.SS(
              paste0(main.dir, "/Scenarios/", input$Scenario_name),
            ...
            show_modal_spinner(
              spin = "flower",
              color = wes_palettes$Moonrise1[3],
              text = "Making plots"
            )
            RPs_4_plots <- as.numeric(trimws(unlist(strsplit(
              input$plot_RPs_inputs,
            ...
            show_modal_spinner(
              spin = "flower",
              color = wes_palettes$Moonrise1[4],
              text = "Making tables"
            )
            #try(SSexecutivesummary(Model.output))
            try(table_all(Model.output))
        ...
        output$SSout_relSB_table <- render_gt({
          SB_indices <- c(
            which(
              rownames(Model.output$derived_quants) ==
                paste0("Bratio_", input$endyr)
            ),
            which(
              rownames(Model.output$derived_quants) == "B_MSY/SSB_unfished"
            ),
            which(
              rownames(Model.output$derived_quants) ==
                paste0("SPRratio_", input$endyr)
            ),
            which(
              rownames(Model.output$derived_quants) ==
                paste0("OFLCatch_", (input$endyr + 1))
            ),
            which(
              rownames(Model.output$derived_quants) ==
                paste0("ForeCatch_", (input$endyr + 1))
            )
          )
          Output_relSB_table <- data.frame(Model.output$derived_quants[
            SB_indices,
            1:3
          ])
          ...
          # 					  ))
          Output_relSB_table[, 1] <- c(
            paste0("SO", input$endyr, "/SO_0"),
            "SO_MSY/SO_0",
            paste0("1-SPR", input$endyr),
            paste0("OFL", (input$endyr + 1)),
            paste0("ABC", (input$endyr + 1))
          )
          Output_relSB_table <- mutate_if(
            Output_relSB_table,
            is.numeric,
            round,
            2
        ...
        output$SSout_F_table <- render_gt({
          F_indices <- c(
            which(
              rownames(Model.output$derived_quants) == paste0("F_", input$endyr)
            ),
        ...
        output$SSout_table <- render_gt({
          Output_table <- Model.output$sprseries |>
            ...
            dplyr::left_join(
              Model.output$annual_time_series,
              by = c("Yr" = "year")
            ) |>
            dplyr::select(
              Yr,
        ...
        output$Parameters_table <- render_gt({
          parm_tab <- cbind(
            rownames(Model.output$estimated_non_dev_parameters),
        ...
        output$Sel_transform_table <- render_gt({
          Est.sel.tranformed <- Model.output$parameters[
            grep("Size_", Model.output$parameters$Label),
            c(2, 3, 8)
          ]
          if (length(grep("peak_", Est.sel.tranformed$Label)) > 0) {
            Est.sel.peak <- Est.sel.tranformed$Value[grep(
              "peak_",
              Est.sel.tranformed$Label
            )]
            Est.sel.tranformed$Value[grep(
              "peak_",
              Est.sel.tranformed$Label
            )] <- Est.sel.peak
          ...
          if (length(grep("ascend_", rownames(Model.output$parameters))) > 0) {
            ...
            Est.sel.tranformed$Value[grep(
              "ascend_",
              Est.sel.tranformed$Label
            )] <- Est.sel.peak - sqrt(-exp(Est.sel.Sel50) * log(0.5))
            Est.sel.tranformed$Label[grep(
              "ascend_",
              Est.sel.tranformed$Label
            )] <- "Length at 50% Selectivity"
          ...
          if (
            ...
            Est.sel.Decline1 <- Est.sel.tranformed$Value[grep(
              "top_logit",
              Est.sel.tranformed$Label
            )]
            Est.sel.tranformed$Value[grep(
          ...
          if (
            ...
            Est.sel.Final <- Est.sel.tranformed$Value[grep(
              "end_logit",
              Est.sel.tranformed$Label
            )]
            Est.sel.tranformed$Value[grep(
              "end_logit",
              Est.sel.tranformed$Label
            )] <- (1 / (1 - exp(-Est.sel.Final))) - 0.000001
            Est.sel.tranformed$Label[grep(
              "end_logit",
              Est.sel.tranformed$Label
            )] <- "Selectivity at max bin size"
  ...
  observeEvent(req(input$run_adnuts), {
    ...
    file.copy(
      list.files(modeff.mod.dir, full.names = TRUE),
      to = file.path(modeff.dir, modeff.name),
      recursive = TRUE,
      overwrite = TRUE
    )
    #}
    ...
    show_modal_spinner(
      spin = "flower",
      color = wes_palettes$Rushmore[1],
      text = paste0("Run ", input$ModEff_choice, " model")
    )
    ...
    if (input$ModEff_choice == "RWM") {
      fit_model <- adnuts::sample_rwm(
        model = m,
        path = p,
        iter = input$iter,
        warmup = 0.25 * input$iter,
        chains = chains,
        thin = input$thin,
        duration = NULL
      )
  ...
  observeEvent(input$run_Profiles, {
    show_modal_spinner(
      spin = "flower",
      color = wes_palettes$Darjeeling1[1],
      text = "Profiles running"
    )
    ...
    SS_parm_names <- c(
      rep.parms.names[grep("steep", rep.parms.names)],
      rep.parms.names[grep("R0", rep.parms.names)],
      rep.parms.names[grep("NatM", rep.parms.names)][1],
      rep.parms.names[grep("L_at_Amax_Fem", rep.parms.names)],
      rep.parms.names[grep("K_Fem", rep.parms.names)],
      rep.parms.names[grep("CV_young_Fem", rep.parms.names)],
      rep.parms.names[grep("CV_old_Fem", rep.parms.names)],
      rep.parms.names[grep("NatM", rep.parms.names)][2],
      rep.parms.names[grep("L_at_Amax_Mal", rep.parms.names)],
      rep.parms.names[grep("K_Mal", rep.parms.names)],
      rep.parms.names[grep("CV_young_Mal", rep.parms.names)],
      rep.parms.names[grep("CV_old_Mal", rep.parms.names)]
    )
    ...
    if (!is.null(parmnames)) {
      ...
      get = get_settings_profile(
        ...
        param_space = rep(
          'real',
          length(as.numeric(trimws(unlist(strsplit(input$Prof_Low_val, ",")))))
        )
      ...
      output$LikeProf_plot_SO <- renderImage(
        {
          image.path3 <- normalizePath(
            file.path(paste0(
              ...
              "/",
              prof_parms_names[1],
              "_trajectories_compare1_spawnbio.png"
            )),
            mustWork = FALSE
          )
          return(list(
      ...
      output$LikeProf_plot_SOt_SO0 <- renderImage(
        {
          image.path4 <- normalizePath(
            file.path(paste0(
              ...
              "/",
              prof_parms_names[1],
              "_trajectories_compare3_Bratio.png"
            )),
            mustWork = FALSE
          )
          return(list(
  ...
  observeEvent(input$run_MultiProfiles, {
    show_modal_spinner(
      spin = "flower",
      color = wes_palettes$Darjeeling1[2],
      text = "Multi-profiles running"
    )
    refdir <- pathLP()
    ...
    par.df <- fread(
      input$file_multi_profile$datapath,
      check.names = FALSE,
      data.table = FALSE
    )
    ...
    SS_parm_names <- rownames(ref.model$parameters)[c(
      23:24,
      1,
      3,
      4:6,
    ...
    profile_dir <- paste0(
      refdir,
      "_profile_",
      paste(prof_parms_names, collapse = "_")
    )
    ...
    file.copy(
      from = file.path(refdir, all_files),
      to = profile_dir,
      overwrite = TRUE
    )
    ...
    profilemodels <- SSgetoutput(
      dirvec = profile_dir,
      keyvec = 1:nrow(par.df),
      getcovar = FALSE
    )
    n <- length(profilemodels)
    profilesummary <- SSsummarize(profilemodels)
    try(SSplotComparisons(
      profilesummary,
    ...
    par.df$likediff <- as.numeric(
      profilesummary$likelihoods[1, 1:n] - ref.model$likelihoods_used[1, 1]
    )
    par.df$Bratio <- as.numeric(profilesummary$Bratio[
      grep((profilesummary$endyrs[1]), profilesummary$Bratio$Label),
      1:n
    ])
    par.df$SB0 <- as.numeric(profilesummary$SpawnBio[1, 1:n])
    par.df$SBcurrent <- as.numeric(profilesummary$SpawnBio[
      grep((profilesummary$endyrs[1]), profilesummary$SpawnBio$Label),
      1:n
    ])
    ...
    colnames(par.df) <- c(
      ...
      c(
        ...
        paste0("SB", profilesummary$endyrs[1], "/SB0"),
        "SB0",
        paste0("SB", profilesummary$endyrs[1])
      )
    )
    save(par.df, file = paste0(profile_dir, "/multiprofilelikelihoods.rds"))
    write.csv(
      par.df,
    ...
    likes_non0_par <- likes_non0_par.min <- profilesummary$likelihoods[
      likes_non0,
    ]
    for (ii in 1:nrow(likes_non0_par)) {
      likes_non0_par.min[ii, 1:nrow(par.df)] <- likes_non0_par[
        ii,
        1:nrow(par.df)
      ] -
        min(likes_non0_par[ii, 1:nrow(par.df)])
    }
    ...
    LC.plot <- ggplot(
      like.comps.plot,
      aes(variable, value, group = Label, color = Label)
    ) +
      geom_point(size = 5) +
      geom_line(lwd = 1.2) +
      ...
      scale_x_discrete(
        name = paste(parmnames[1], "and", parmnames[2]),
        breaks = par.df[, 1],
        labels = paste0(par.df[, 1], "\n", par.df[, 2])
      )
    ggsave(
      paste0(profile_dir, "/", "like_component_profile.png"),
      plot = LC.plot,
      width = 10,
      height = 10,
      units = "in"
    ...
    if (nrow(likes_length) > 0) {
      ...
      likes_length_non0 <- likes_length_non0.min <- data.frame(
        model = likes_length[, 1],
      ...
      for (ii in 2:ncol(likes_length_non0)) {
        likes_length_non0.min[, ii] <- likes_length_non0[, ii] -
          min(likes_length_non0.min[, ii])
      }
      likes_length_non0.min.melt <- reshape2::melt(
      ...
      LC_lt.plot <- ggplot(
        ...
        aes(model, value, group = variable, color = variable)
      ) +
        geom_point(size = 5) +
        geom_line(lwd = 1.2) +
        theme(legend.position = c(0.8, 0.8)) +
      ...
      ggsave(
        paste0(profile_dir, "/", "length_component_profile.png"),
        plot = LC_lt.plot,
        width = 10,
        height = 10,
        units = "in"
    ...
    if (nrow(likes_age) > 0) {
      ...
      likes_age_non0 <- likes_age_non0.min <- data.frame(
        model = likes_age[, 1],
      ...
      for (ii in 2:ncol(likes_age_non0)) {
        likes_age_non0.min[, ii] <- likes_age_non0[, ii] -
          min(likes_age_non0.min[, ii])
      }
      likes_age_non0.min.melt <- reshape2::melt(
        likes_age_non0.min,
        id.vars = "model"
      )
      ...
      ggsave(
        paste0(profile_dir, "/", "age_component_profile.png"),
        plot = LC_age.plot,
        width = 10,
        height = 10,
        units = "in"
    ...
    if (nrow(likes_survey) > 0) {
      ...
      likes_survey_non0 <- likes_survey_non0.min <- data.frame(
        model = likes_survey[, 1],
      ...
      for (ii in 2:ncol(likes_survey_non0)) {
        likes_survey_non0.min[, ii] <- likes_survey_non0[, ii] -
          min(likes_survey_non0.min[, ii])
      }
      likes_survey_non0.min.melt <- reshape2::melt(
      ...
      ggsave(
        paste0(profile_dir, "/", "survey_component_profile.png"),
        plot = LC_survey.plot,
        width = 10,
        height = 10,
        units = "in"
    ...
    blank_data <- data.frame(
      ...
      x = min(par.df[, 1]),
      y = c(
        min(par.df$Likelihood_difference),
        max(par.df$Likelihood_difference),
        0,
        1,
        0,
        ceiling(max(par.df$SB0)),
        0,
        ceiling(SBcurrmax)
      )
    )
    blank_data$variable <- factor(
      blank_data$variable,
      c(
        "Likelihood_difference",
        paste0("SB", profilesummary$endyrs[1], "/SB0"),
    ...
    refmodel.dat <- data.frame(
      ...
      x = ref.model$parameters[
        grep(prof_parms_names[1], ref.model$parameters$Label),
        3
      ],
      y = c(
        0,
        ref.model$sprseries$Deplete[
          grep((profilesummary$endyrs[1]), profilesummary$Bratio$Label) + 1
        ],
        ref.model$SBzero,
        ref.model$derived_quants[
          grep((profilesummary$endyrs[1]), profilesummary$SpawnBio$Label),
          2
        ]
      )
    ...
    output$LikeProf_multiplot <- renderPlot({
      multiplot <- ggplot(plot.dat(), aes(plot.dat()[, 1], value)) +
        ...
        geom_hline(
          ...
          aes(yintercept = yint),
          linetype = c("solid", "dotted", "solid", "dotted", "solid"),
          color = c("red", "black", "red", "darkgreen", "red"),
          lwd = 1
        ) +
        geom_point(
          data = refmodel.dat,
        ...
        theme_bw()
      ggsave(
        paste0(profile_dir, "/", "multilikelihood_profile.png"),
        width = 10,
        height = 10,
        units = "in"
      )
    ...
    output$Retro_comp_plotSB <- renderImage(
      {
        image.path <- normalizePath(
          file.path(paste0(
            pathRetro(),
            "_retro/compare2_spawnbio_uncertainty.png"
          )),
          mustWork = FALSE
        )
        return(list(
    ...
    output$Retro_comp_plotBratio <- renderImage(
      {
        image.path <- normalizePath(
          file.path(paste0(
            pathRetro(),
            "_retro/compare4_Bratio_uncertainty.png"
          )),
          mustWork = FALSE
        )
        return(list(
  ...
  Sensi_model_dir_out <- eventReactive(
    req(
      input$run_Sensi_comps &
        !is.null(input$myPicker) &
        as.numeric(input$tabs) == 6
    ),
    {
  ...
  observeEvent(req(input$run_Sensi_comps), {
    ...
    pngfun(
      ...
      file = paste0(input$Sensi_comp_file, ".png"),
      h = 7,
      w = 12
    )
    par(mfrow = c(1, 3))
    ...
    pngfun(
      ...
      file = paste0(input$Sensi_comp_file, "_no_uncertainty.png"),
      h = 7,
      w = 12
    )
    par(mfrow = c(1, 3))
    ...
    SensiRE_xcenter_in <- as.numeric(trimws(unlist(strsplit(
      input$SensiRE_xcenter,
      ","
    ))))
    SensiRE_ycenter_in <- as.numeric(trimws(unlist(strsplit(
      input$SensiRE_ycenter,
      ","
    ))))
    ...
    try(Sensi_plot_horiz(
      ...
      #Sensi.RE.out="Sensi_RE_out.rds", #Saved file of relative errors
    ...
    output$SensiRE_comp_plot <- renderImage(
      {
        image.path <- normalizePath(
          file.path(paste0(
            ...
            "/Sensitivity Comparison Plots/",
            input$Sensi_comp_file,
            "/Sensi_REplot_SB_Dep_F_Yield.png"
          )),
          mustWork = FALSE
        )
        return(list(
    ...
    output$SensiRElog_comp_plot <- renderImage(
      {
        image.path <- normalizePath(
          file.path(paste0(
            ...
            "/Sensitivity Comparison Plots/",
            input$Sensi_comp_file,
            "/Sensi_logREplot_SB_Dep_F_Yield.png"
          )),
          mustWork = FALSE
        )
        return(list(
  ...
  observeEvent(
    ...
    ),
    {
      ...
      for (i in 1:Runs) {
        zz[[i]] <- SS_output(paste0(pathEnsemble(), "/", input$myEnsemble[i]))
      }
      modsummary.ensemble <- SSsummarize(zz)
      Ensemble_wts <- as.numeric(trimws(unlist(strsplit(
        input$Ensemble_wts,
        ","
      ))))
      ...
      SO_0 <- SO_t <- Bratio_t <- F_t <- SPR_t <- data.frame(
        Year = NA,
        Metric = NA,
        Model = NA
      )
      ...
      for (i in 1:length(Nsamps_ensemble_wts)) {
        ...
        SpOt_en[[i]] <- Map(
          mean.fxn,
          modsummary.ensemble$SpawnBio[, i],
          modsummary.ensemble$SpawnBioSD[, i]
        )
        names(SpOt_en[[i]]) <- modsummary.ensemble$SpawnBio$Yr
        SO_0 <- rbind(
          SO_0,
          data.frame(
            Year = as.numeric(names(SpOt_en[[i]][1])),
            Metric = unlist(SpOt_en[[i]][1]),
            Model = input$myEnsemble[i]
          )
        )
        SO_t <- rbind(
          SO_t,
          data.frame(
            Year = names(SpOt_en[[i]][nrow(modsummary.ensemble$SpawnBio)]),
            Metric = unlist(SpOt_en[[i]][nrow(modsummary.ensemble$SpawnBio)]),
            Model = input$myEnsemble[i]
          )
        )
        Bratio_en[[i]] <- Map(
          mean.fxn,
          modsummary.ensemble$Bratio[, i],
          modsummary.ensemble$BratioSD[, i]
        )
        names(Bratio_en[[i]]) <- modsummary.ensemble$Bratio$Yr
        Bratio_t <- rbind(
          Bratio_t,
          data.frame(
            Year = names(Bratio_en[[i]][nrow(modsummary.ensemble$Bratio)]),
            Metric = unlist(Bratio_en[[i]][nrow(modsummary.ensemble$Bratio)]),
            Model = input$myEnsemble[i]
          )
        )
        F_en[[i]] <- Map(
          mean.fxn,
          modsummary.ensemble$Fvalue[, i],
          modsummary.ensemble$FvalueSD[, i]
        )
        names(F_en[[i]]) <- modsummary.ensemble$Fvalue$Yr
        F_t <- rbind(
          F_t,
          data.frame(
            Year = names(F_en[[i]][nrow(modsummary.ensemble$Fvalue)]),
            Metric = unlist(F_en[[i]][nrow(modsummary.ensemble$Fvalue)]),
            Model = input$myEnsemble[i]
          )
        )
        SPR_en[[i]] <- Map(
          mean.fxn,
          modsummary.ensemble$SPRratio[, i],
          modsummary.ensemble$SPRratioSD[, i]
        )
        names(SPR_en[[i]]) <- modsummary.ensemble$SPRratio$Yr
        SPR_t <- rbind(
          SPR_t,
          data.frame(
            Year = names(SPR_en[[i]][nrow(modsummary.ensemble$SPRratio)]),
            Metric = unlist(SPR_en[[i]][nrow(modsummary.ensemble$SPRratio)]),
            Model = input$myEnsemble[i]
          )
        )
      }
      ...
      colnames(Ensemble_Bratio) <- colnames(Ensemble_F) <- colnames(
        Ensemble_SPR
      ) <- names(Bratio_en[[1]])
      ...
      SO_0 <- rbind(
        SO_0[-1, ],
        data.frame(
          Year = as.numeric(colnames(Ensemble_SO)[1]),
          Metric = Ensemble_SO[, 1],
          Model = "Ensemble"
        )
      )
      SO_t <- rbind(
        SO_t[-1, ],
        data.frame(
          Year = as.numeric(colnames(Ensemble_SO)[ncol(Ensemble_SO)]),
          Metric = Ensemble_SO[, ncol(Ensemble_SO)],
          Model = "Ensemble"
        )
      )
      Bratio_t <- rbind(
        Bratio_t[-1, ],
        data.frame(
          Year = as.numeric(colnames(Ensemble_Bratio)[ncol(Ensemble_Bratio)]),
          Metric = Ensemble_Bratio[, ncol(Ensemble_Bratio)],
          Model = "Ensemble"
        )
      )
      F_t <- rbind(
        F_t[-1, ],
        data.frame(
          Year = as.numeric(colnames(Ensemble_F)[ncol(Ensemble_F)]),
          Metric = Ensemble_F[, ncol(Ensemble_F)],
          Model = "Ensemble"
        )
      )
      SPR_t <- rbind(
        SPR_t[-1, ],
        data.frame(
          Year = as.numeric(colnames(Ensemble_SPR)[ncol(Ensemble_SPR)]),
      ...
      gg3 <- ggplot(Bratio_t, aes(Model.labs, Metric, fill = Model)) +
        ...
        scale_y_continuous(limits = c(0, NA)) +
        geom_hline(
          yintercept = c(input$Ensm_TRP, input$Ensm_LRP),
          col = c("green", "red"),
      ...
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_comp_plots_SOterm.png"),
        plot = gg2,
        width = 20,
        height = 10
      )
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_comp_plots_RSS.png"),
        plot = gg3,
        width = 20,
        height = 10
      )
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_comp_plots_F.png"),
        plot = gg4,
        width = 20,
        height = 10
      )
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_comp_plots_SO_1_SPR.png"),
        plot = gg5,
        width = 20,
        height = 10
      )
      ...
      xlab.yr.short <- NA
      xlab.yr.short[
        as.character(modsummary.ensemble$SpawnBio$Yr) %in%
      ...
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_SO.png"),
        width = 20,
        height = 10
      )
      ...
      Bratio.ts.plot <- ggplot(
        Ensemble_Bratio_plot,
        aes(Year, Bratio, fill = Year)
      ) +
        geom_violin(bw = 0.01) +
        theme(legend.position = "none") +
        #theme(axis.text.x = element_text(angle = 65, hjust = 0.1,vjust=0))+
        ...
        theme(axis.text = element_text(size = 14))
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_Bratio.png"),
        width = 20,
        height = 10
      )
      ...
      ggplot(Ensemble_F_plot, aes(Year, F, fill = Year)) +
        ...
        ylab("Fishing mortality")
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_F.png"),
        width = 20,
        height = 10
      )
      ...
      ggplot(Ensemble_SPR_plot, aes(Year, SPR, fill = Year)) +
        ...
        ylab("1-SPR")
      ggsave(
        paste0(Ensemble_model_dir_out, "/Ensemble_SPR.png"),
        width = 20,
        height = 10
      )
      ...
      show_modal_spinner(
        spin = "flower",
        color = wes_palettes$Rushmore[3],
        text = "Saving ensemble objects"
      )
      Model.outputs <- list(
        "Spawning Output" = SpOt_en,
        "Relative Stock Status" = Bratio_en,
        "Fishing mortality" = F_en,
        "1-SPR" = SPR_en
      )
      Ensemble.outputs <- list(
        "Spawning Output" = Ensemble_SO,
        "Relative Stock Status" = Ensemble_Bratio,
        "Fishing mortality" = Ensemble_F,
        "1-SPR" = Ensemble_SPR
      )
      Ensemble.outputs.plots <- list(
        "Spawning Output" = Ensemble_SO_plot,
        "Relative Stock Status" = Ensemble_Bratio_plot,
        "Fishing mortality" = Ensemble_F_plot,
        "1-SPR" = Ensemble_SPR_plot
      )
      save(
        Model.outputs,
        file = paste0(Ensemble_model_dir_out, "/Model_results", ".rds")
      )
      save(
        Ensemble.outputs,
        file = paste0(Ensemble_model_dir_out, "/Ensemble_results", ".rds")
      )
      save(
  ...
  onRestored(function(state) {
    output$input_file_text <- renderText({
      paste0(
        "<span style=\"color:#5D9741\">Restored session from ",
        basename(state$dir),
        ".rds</span>"
      )
    })
