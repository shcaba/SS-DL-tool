...
#    fluidRow(column(width=10,checkboxInput("mod_sims","Specify model years?",FALSE))),
    wellPanel(
    h3("Model years"),
    h5(p(em("Starting values based on inputs"))),
 ...
 #   ),
  ...
  shinyjs::hidden(wellPanel(id="panel_SS_LH_fixed",
    h3("Life history inputs"),
        ...
        h5(em("FEMALE")),
        fluidRow(column(width=6,numericInput("Nages_fix","Max. age", value=NA,min=1, max=1000, step=1)),
                column(width=6,numericInput("M_f_fix", "Natural mortality", value=NA,min=0, max=10000, step=0.01))),    
