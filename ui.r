...
shinyUI(fluidPage(theme = "bootstrap.css",
  ...
  titlePanel("Welcome to the Stock Synthesis data-limited tool (SS-DL tool)"),
      h5(p(strong("This tool uses the Stock Synthesis framework to implement the following types of models:"))),
       ...
       tags$ul(tags$li(h5(p(em("Length + catch-based (SS-CL) estimation of biomass, relative biomass, SPR and F and catch limits."))))),
       tags$ul(tags$li(h5(p(em("Age-structure surplus production model (SS-SP) estimation of biomass, relative biomass, SPR and F and catch limits."))))),
       tags$ul(tags$li(h5(p(em("Statistical catch at age model (catches, indices and length and/or age compositions) estimation of biomass, relative biomass, SPR and F and catch limits."))))),
       tags$ul(tags$li(h5(p(em("COMING SOON: Extended Simple Stock Synthesis using AIS (XSSS-AIS) estimator of sustainable catch."))))),
sidebarLayout(
   sidebarPanel(
 ...
 fluidRow(column(width=12,fileInput('file2', 'Catch time series',
  ...
  h5(em("Make sure the model run is in the Scenarios folder. Put that folder name in the Scenario name input and run the model.")),
    ...
    shinyjs::hidden(wellPanel(id="panel_Ct_F_LO",
        ...
        h5(em("Fishing rate can also be directly estimated from the length composition with no assumption in catches.")),
    ...
    h4(strong("Life history inputs")),
        ...
        fluidRow(column(width=6,numericInput("t0_f","Age at length 0 (t0)", value=NA,min=-100, max=10000, step=0.01)),
...
shinyjs::hidden(wellPanel(id="panel_SS_LH_fixed_est_tog",
  ...
  shinyjs::hidden(wellPanel(id="panel_SS_LH_fixed",
    h4(strong("Life history inputs")),
        ...
        h5("Fecundity-length relationship F=aL^b. Set equal to the W-L relationship to get fecundity equivalent to spawning biomass"),
        fluidRow(column(width=6,numericInput("Fec_a_f_fix", "Coefficent a", value=0.00001,min=0, max=10000, step=-0.01)),
                column(width=6,numericInput("Fec_b_f_fix","Exponent b", value=3,min=0, max=10000, step=0.01))),
        fluidRow(column(width=6,numericInput("WLa_f_fix", "Weight (kg)-Length (cm): a in W=aL^b", value=0.00001,min=0, max=10000, step=0.000000001)),
                column(width=6,numericInput("WLb_f_fix","Weight (kg)-Length (cm): b in W=aL^b", value=3,min=0, max=10000, step=0.01))),    
...
#      fluidRow(column(width=10,switchInput("est_parms2","Estimate parameters?",value=TRUE))),      
     ...
     br(),
       dropdownButton(
          ...
          numericInput("CV_lt_f_young_mean", "Mean", value=0.1,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_young_SD", "SD", value=0,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_young_phase", "Phase", value=-1,min=-999, max=10, step=0.001),
     ...
     br(),
       dropdownButton(
          ...
          numericInput("CV_lt_f_old_mean", "Mean", value=0.1,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_old_SD", "SD", value=0,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_old_phase", "Phase", value=-1,min=-999, max=10, step=0.001),
    ...
    h5(strong("Maturity and weight-length relationships")),            
      fluidRow(column(width=6,numericInput("L50_f_est", "Length at 50% maturity", value=NA,min=0, max=10000, step=0.01)),
        ...
        h5("Fecundity-length relationship F=aL^b. Set equal to the W-L relationship to get fecundity equivalent to spawning biomass"),
        fluidRow(column(width=6,numericInput("Fec_a_f_est", "Coefficient a", value=0.00001,min=0, max=10000, step=-0.01)),
              column(width=6,numericInput("Fec_b_f_est","Exponent b", value=3,min=0, max=10000, step=0.01))),    
   ...
   #  ),
    ...
    h5(em("Length CV")),            
      dropdownButton(
          ...
          numericInput("CV_lt_f_young_mean_sss", "Mean", value=0.1,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_young_SD_sss", "SD", value=0,min=0, max=10000, step=0.001),
      ...
      dropdownButton(
          ...
          numericInput("CV_lt_f_old_mean_sss", "Mean", value=0.1,min=0, max=10000, step=0.001),
          numericInput("CV_lt_f_old_SD_sss", "SD", value=0,min=0, max=10000, step=0.001),
          circle = FALSE, right=TRUE, status = "danger", icon = icon("dice"), width = "300px",label="CV at length"
            ),
    h5(strong("Maturity and weight-length relationships")),            
      ...
      h5("Fecundity-length relationship F=aL^b. Set equal to the W-L relationship to get fecundity equivalent to spawning biomass"),
      fluidRow(column(width=6,numericInput("Fec_a_f_sss", "Cofficient a", value=0.00001,min=0, max=10000, step=-0.01)),
              column(width=6,numericInput("Fec_b_f_sss","Exponent b", value=3,min=0, max=10000, step=0.01))),    
      fluidRow(column(width=6,numericInput("WLa_f_sss", "Weight (kg)-Length (cm): a in W=aL^b", value=0.00001,min=0, max=10000, step=0.000000001)),
...
###############################
  shinyjs::hidden(wellPanel(id="Sensi_Comparison_panel",
    ...
    fluidRow(column(width=8,textInput("SensiRE_headers", strong("Headers for relative error plots"), value="0"))),
    fluidRow(column(width=6,textInput("SensiRE_xcenter", "Vertical (x) centering of headers", value="0")),
